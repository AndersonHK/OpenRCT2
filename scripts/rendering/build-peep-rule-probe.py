"""Build only the diagnostic peep compute entry point, with strict receipts.
...
INPUTS = (
    ...
    'test/tests/PeepRulesTests.cpp',
    'test/peep-parity/FrozenPeepRules.inc',
    'test/peep-parity/FrozenPeepRules.json',
    'test/peep-parity/PeepRulesProbe.comp',
)
...
def main():
    ...
    shader = output / 'PeepRulesProbe.comp.spv'
    verifier = root / 'scripts/rendering/verify-peep-rule-oracle.py'
    ...
    receipt = {'schema': 1, 'status': 'pass' if passed else 'fail', 'commands': commands, 'exitCodes': codes,
               ...
               'scope': 'Diagnostic-only compute shader compile; no GPU execution, production shader registration or peep admission.'}
