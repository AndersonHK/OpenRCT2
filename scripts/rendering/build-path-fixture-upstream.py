"""Build a path fixture/reference driver against the pinned pristine upstream core.
...
SPEC = importlib.util.spec_from_file_location('path_upstream_build', HELPER)
...
def main():
    ...
    receipt = dict(schema=1, kind='upstream-path-fixture-build', status='fail',
    ...
    try:
        ...
        original = ROOT / 'test/path-parity/PathFixtureMain.cpp'
        ...
        H.require(result.returncode == 0 and (binary / 'path-fixture.exe').is_file(), 'Driver build failed')
        passed = True
