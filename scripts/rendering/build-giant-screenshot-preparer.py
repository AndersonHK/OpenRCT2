"""Build an isolated, public-API terrain preparer against frozen or current core.
...
def inputs(source, harness):
    ...
    result.update({"harness/" + path.relative_to(harness).as_posix(): build.sha256(path)
                   for path in sorted((harness / "test/terrain-parity").rglob("*")) if path.is_file()})
    recipe_header = harness / "test/terrain-parity/GiantScreenshotRecipe.h"
    if not recipe_header.is_file():
        raise RuntimeError("Missing immutable nonuniform terrain recipe header")
    census_header = harness / "test/ui-parity/BalloonFixtureState.h"
    result["harness/test/ui-parity/BalloonFixtureState.h"] = build.sha256(census_header)
    return result
...
def driver_project(source, harness, output, frozen, vulkan):
    ...
    child(properties, "ProjectName", "giant-screenshot-preparer")
    ...
    if frozen:
        child(compiler, "PreprocessorDefinitions", "NATIVE_TERRAIN_ALLOW_PREPARE;%(PreprocessorDefinitions)")
    elif vulkan:
    ...
    child(sources, "ClCompile", Include=str(harness / "test/terrain-parity/GiantScreenshotFixtureMain.cpp"))
    child(project, "Import", Project=r"$(VCTargetsPath)\Microsoft.Cpp.targets")
    path = output / "giant-screenshot-driver.vcxproj"
...
def main():
    ...
    with (output / "build.log").open("w", encoding="utf-8") as log:
        for name, project, target in (("core", core, "libopenrct2"), ("driver", driver, "giant-screenshot-preparer")):
    ...
    artifacts = [output / "bin/libopenrct2.lib", output / "bin/giant-screenshot-preparer.exe"]
    ...
    receipt = {"schema": 1, "status": "pass" if passed else "fail", "exitCode": result_code,
               ...
               "recipeHeaderSha256": before["harness/test/terrain-parity/GiantScreenshotRecipe.h"],
