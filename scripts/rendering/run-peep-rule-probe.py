"""Run receipt-pinned peep compute readback in a fresh no-window process."""
import argparse
...
spec = importlib.util.spec_from_file_location('peep_probe_builder', Path(__file__).with_name('build-peep-rule-probe.py'))
builder = importlib.util.module_from_spec(spec)
spec.loader.exec_module(builder)
sha = builder.sha
CASES = {
    'PublishedFieldsAfterSkippedRevisionsMatchFrozenPainter': 1,
    'ActualDeviceSelectorsProjectionAndAccessoriesMatchFrozenPainter': 30720,
    'InvalidCatalogAndDispatchBoundariesPreserveGuards': 2646,
}
...
def main():
    ...
    summary = {'schema': 1, 'status': 'fail', 'exitCode': None, 'timedOut': False,
               ...
               'scope': 'Actual no-window compute of the shared peep GLSL selector and projection with a separate fact SSBO. '
                        'Image/remap/accessory and projection expectations use pinned frozen painter bodies. '
                        'Descriptor validation, input immutability and over-dispatch guards are checked. '
                        'No sprite pixels, atlas/zoom sampling, interpolation, staff lighting, mixed-world ordering, '
                        'production admission or performance qualification.'}
    runtime_inputs = {}
    ...
    try:
        ...
        shader_name = 'PeepRulesProbe.comp.spv'
        ...
        command = [str(executable), '--gtest_filter=VulkanPeepRulesTest.*', '--gtest_output=xml:' + str(xml)]
        ...
        pin(settings)
        diagnostic_env = {'OPENRCT2_REQUIRE_VULKAN_TESTS': '1', 'OPENRCT2_PEEP_RULE_PROBE_SPV': str(shader),
                          'OPENRCT2_PEEP_RULE_ARTIFACTS': str(output / 'samples'),
        ...
        if len(cases) != len(CASES) or set(names) != set(CASES):
            summary['failures'].append('Expected exactly all declared device tests')
        for case in cases:
            name = case.get('name')
            if case.get('classname') != 'VulkanPeepRulesTest' or case.get('status') != 'run' or any(
            ...
            count = int(props.get('peepQueryCount', '0'))
            dispatches = int(props.get('peepDispatchCount', '0'))
            ...
            sizes = {'queries.bin': 168, 'gpu-results.bin': 112, 'expected-contract.bin': 112, 'actual-contract.bin': 112}
