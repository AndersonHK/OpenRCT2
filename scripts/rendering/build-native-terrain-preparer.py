...
def inputs(source, harness):
    ...
    recipe_header = harness / "test/terrain-parity/NonuniformTerrainRecipe.h"
...
def driver_project(source, harness, output, frozen, vulkan):
    ...
    child(properties, "ProjectName", "native-terrain-preparer")
    ...
    child(sources, "ClCompile", Include=str(harness / "test/terrain-parity/NativeTerrainFixtureMain.cpp"))
    child(project, "Import", Project=r"$(VCTargetsPath)\Microsoft.Cpp.targets")
    path = output / "native-terrain-driver.vcxproj"
...
def main():
    ...
    with (output / "build.log").open("w", encoding="utf-8") as log:
        for name, project, target in (("core", core, "libopenrct2"), ("driver", driver, "native-terrain-preparer")):
    ...
    artifacts = [output / "bin/libopenrct2.lib", output / "bin/native-terrain-preparer.exe"]
    ...
    receipt = {"schema": 1, "status": "pass" if passed else "fail", "exitCode": result_code,
               ...
               "recipeHeaderSha256": before["harness/test/terrain-parity/NonuniformTerrainRecipe.h"],
