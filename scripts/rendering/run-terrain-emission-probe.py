...
spec = importlib.util.spec_from_file_location('terrain_probe_builder', Path(__file__).with_name('build-terrain-emission-probe.py'))
builder = importlib.util.module_from_spec(spec)
spec.loader.exec_module(builder)
sha = builder.sha
CASES = {
    'RecipeDescriptorsAndRetentionMatchFrozenTrace': [35136] + [0] * 16 + [8192, 8192, 2368, 35136, 35136],
    'InvalidStateAndCapacityEmitNothing': [35136, 0, 8192, 10560, 2368],
}
...
def main():
    ...
    summary = {'schema': 1, 'status': 'fail', 'exitCode': None, 'timedOut': False,
               ...
               'scope': 'Actual graphics-only retained state upload and compute descriptor readback. '
                        'Frozen edge/shape oracle and current public material selector; complete output-capacity poison guards. '
                        'No terrain pixels, cross-tile ordering, runtime publication/admission or performance qualification.'}
    runtime_inputs = {}
    ...
    try:
        ...
        shader_name = 'terrain_retained_emit.comp.spv'
        ...
        command = [str(executable), '--gtest_filter=VulkanRetainedTerrainEmissionTest.*', '--gtest_output=xml:' + str(xml)]
        ...
        pin(settings)
        diagnostic_env = {'OPENRCT2_REQUIRE_VULKAN_TESTS': '1', 'OPENRCT2_TERRAIN_EMISSION_SPV': str(shader),
                          'OPENRCT2_TERRAIN_EMISSION_ARTIFACTS': str(output / 'samples'),
        ...
        for case in cases:
            name = case.get('name')
            if case.get('classname') != 'VulkanRetainedTerrainEmissionTest' or case.get('status') != 'run' or any(
            ...
            count = int(props.get('retainedTerrainSamples', '0'))
            expected_uploads = CASES.get(name, [])
            if count != len(expected_uploads) or not expected_uploads:
                summary['failures'].append('Incomplete retained state sample coverage: ' + str(name))
                continue
            samples = output / 'samples' / name
            if {p.name for p in samples.iterdir()} != {str(i) for i in range(count)}:
                raise RuntimeError('Unexpected retained sample directory set: ' + name)
            reports = []
            for index in range(count):
                folder = samples / str(index)
                names = ('sample.json', 'before.bin', 'after.bin', 'expected.bin', 'status.bin', 'tiles.bin', 'materials.bin', 'gpu-tiles.bin', 'gpu-materials.bin')
                if {p.name for p in folder.iterdir()} != set(names):
                    raise RuntimeError('Missing or unexpected retained artifact: ' + str(folder))
                metadata = json.loads((folder / 'sample.json').read_text(encoding='utf-8'))
                before, after, expected, status_bytes = ((folder / p).read_bytes() for p in
                    ('before.bin', 'after.bin', 'expected.bin', 'status.bin'))
                if len(before) != 8192 * 64 or len(after) != len(before) or any(v != 0xa5 for v in before):
                    raise RuntimeError('Malformed full-capacity guard buffers: ' + str(folder))
                if len(expected) % 64 or len(expected) != metadata['expectedPrimitiveCount'] * 64 or len(status_bytes) != 48:
                    raise RuntimeError('Malformed typed descriptor/status output: ' + str(folder))
                status = struct.unpack('<12I', status_bytes)
                error = metadata['expectedError']
                valid = error == 0
                prefix = len(expected) if valid else 0
                if status[0] != error or status[1] != (len(expected) // 64 if valid else 0) or status[8:] != (0,0,0,0):
                    raise RuntimeError('GPU status or closed-admission contract mismatch: ' + str(folder))
                if valid and (after[:prefix] != expected or status[3] != status[1] or sum(status[4:8]) != status[1]):
                    raise RuntimeError('Frozen descriptor/count oracle differs from actual GPU: ' + str(folder))
                if after[prefix:] != before[prefix:]:
                    raise RuntimeError('Rejected output or unused capacity was written: ' + str(folder))
                if metadata['uploadedWorldBytes'] != expected_uploads[index]:
                    raise RuntimeError('Retained revision upload-byte contract differs: ' + str(folder))
                if (folder / 'tiles.bin').stat().st_size != 1024 * 32 or (folder / 'materials.bin').stat().st_size != 4 * 592:
                    raise RuntimeError('Raw retained input ABI differs: ' + str(folder))
                for raw, gpu in (('tiles.bin', 'gpu-tiles.bin'), ('materials.bin', 'gpu-materials.bin')):
                    if (folder / raw).read_bytes() != (folder / gpu).read_bytes():
                        raise RuntimeError('Retained GPU input buffer differs from immutable snapshot: ' + str(folder))
                reports.append({'sample': index, 'metadata': metadata,
                    'artifactSha256': {filename: sha(folder / filename) for filename in names}})
            summary['reports'][name] = {'samples': count, 'reports': reports}

    except (OSError, RuntimeError, ValueError, KeyError, TypeError, OverflowError, struct.error, ET.ParseError) as error:
