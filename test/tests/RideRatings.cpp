...
{
protected:
    const TileCoordsXY originTile{ 10, 10 };
    const RideId rideId = RideId::FromUnderlying(1);

    struct CachedSceneryContext
    {
        std::unique_ptr<IContext> context;
        TileCoordsXY sceneryTile;
        CoordsXYZ origin;
        SurfaceElement* scenerySurface{};
    };

    template<typename T>
    static void ExpectTickScore(const T& actual, const RideRating::TickScore& expected)
    {
        EXPECT_EQ(actual.excitement, expected.excitement);
    ...
    static RideRating::TickScore ScoreLocalContext(
        const RideRating::LocalContextScore& context, int32_t speed, int32_t coefficient = 1000, bool isBoatHire = false)
    {
    ...
    static void InitialiseFixedRide(Ride& ride, RideId id, ride_type_t type, const TileCoordsXY& stationTile, int32_t baseZ)
    {
        ride.id = id;
        ride.type = type;
        ride.numStations = 1;
        ride.getStation().setStart(stationTile.toCoordsXY());
    ...
    {
        ride.type = type;
        ride.numStations = numStations;
    }

    static void InitialiseStableTransportRide(Ride& ride, uint8_t numStations)
    {
        InitialiseRide(ride, RIDE_TYPE_MONORAIL, numStations);
        ride.stableStats.valid = true;
        ride.stableStats.averageSpeed = 18 * 29127;
        ride.stableStats.maxSpeed = ride.stableStats.averageSpeed;
    }

    static Ride* AllocateStableTransportRide(RideId id, uint8_t numStations)
    {
        auto* ride = RideAllocateAtIndex(id);
        EXPECT_NE(ride, nullptr);
        if (ride != nullptr)
        {
            InitialiseStableTransportRide(*ride, numStations);
            ride->status = RideStatus::open;
        }
        return ride;
    }

    static void AddMeasuredTransportLeg(
        Ride& ride, uint8_t origin, uint8_t destination, int32_t distance, uint32_t ticks = 40, int32_t speed = 0x80000,
        int32_t maxSpeed = 0x90000, bool sheltered = false)
    {
        ...
        sample.transportShelteredDistance = sheltered ? sample.transportDistance : 0;
        sample.sampledDistance = static_cast<int64_t>(distance) << 16;
        sample.totalSpeed = static_cast<int64_t>(speed) * ticks;
        sample.maxSpeed = maxSpeed;
        sample.ticks = ticks;
        RideAddRecentRatingSample(ride, sample);
    }

    static RideRatingAccumulator MakeRatingSample(
        int64_t excitement, int64_t intensity, int64_t nausea, uint32_t ticks, StationIndex origin = StationIndex::GetNull(),
        StationIndex destination = StationIndex::GetNull())
    {
        RideRatingAccumulator sample{};
        sample.excitement = excitement;
        sample.intensity = intensity;
        sample.nausea = nausea;
        sample.ticks = ticks;
        sample.originStation = origin;
        sample.destinationStation = destination;
        return sample;
    }

    std::unique_ptr<IContext> InitialiseContext()
    {
        gOpenRCT2Headless = true;
        gOpenRCT2NoGraphics = true;
        auto context = CreateContext();
        EXPECT_TRUE(context->Initialise());
        return context;
    }

    std::unique_ptr<IContext> InitialiseMap()
    {
        auto context = InitialiseContext();
        MapInit({ 30, 30 });
        return context;
    }

    std::unique_ptr<IContext> LoadPark(std::string name, bool initialiseGame = false)
    {
        auto context = InitialiseContext();
        GetContext()->LoadParkFromFile(TestData::GetParkPath(std::move(name)));
        if (initialiseGame)
        {
            GameLoadInit();
        }
    ...
    CachedSceneryContext PrepareCachedSceneryContext()
    {
        CachedSceneryContext result{ .context = LoadPark("tile-element-tests.sv6", true) };
        result.sceneryTile = FindGrassSurfaceTile(8, 8);
        EXPECT_GE(result.sceneryTile.x, 1);
        const auto contextOriginTile = TileCoordsXY{ result.sceneryTile.x - 1, result.sceneryTile.y };
        const auto* originSurface = MapGetSurfaceElementAt(contextOriginTile);
        EXPECT_NE(originSurface, nullptr);
        result.scenerySurface = MapGetSurfaceElementAt(result.sceneryTile);
    ...
    {
        auto* mazeElement = InsertTileElement<TrackElement>({ tile.toCoordsXY(), baseZ }, 0, [&](TrackElement& mazeElement) {
    ...
    {
        auto* trackElement = InsertTileElement<TrackElement>({ tile.toCoordsXY(), baseZ }, 0, [&](TrackElement& trackElement) {
    ...
    CoordsXYZ PlaceVehicleTrack(
        const TileCoordsXY& tile, int32_t trackZ, TrackElemType trackType = TrackElemType::flatTrack1x4A,
    ...
    CoordsXYZ PlaceVehicleTrackWithTerrain(
        const TileCoordsXY& tile, int32_t trackZ, int32_t firstSideZ, int32_t secondSideZ,
        TrackElemType trackType = TrackElemType::flatTrack1x4A, Direction direction = 0)
    {
        SetSurfaceZ(tile, trackZ);
        SetVehicleSideSurfaces(tile, direction, firstSideZ, secondSideZ);
        return PlaceVehicleTrack(tile, trackZ, trackType, direction);
    }

    void PlacePath(const TileCoordsXY& tile, int32_t baseZ, int32_t clearanceZ, uint8_t edges = 0)
    {
        auto* pathElement = InsertTileElement<PathElement>({ tile.toCoordsXY(), baseZ }, 0, [&](PathElement& pathElement) {
    ...
    static bool IsAggregateSummaryStatGate(RatingsModifierType type)
    {
        switch (type)
        {
    ...
    Ride* FindNormalRide(bool requireSummaryStatGate = false)
    {
        auto& gameState = getGameState();
        for (auto& ride : RideManager(gameState))
        {
            const auto& rtd = ride.getRideTypeDescriptor();
            const bool hasRequiredGate = !requireSummaryStatGate
                || std::ranges::any_of(
                    rtd.RatingsData.Modifiers, [](const auto& modifier) { return IsAggregateSummaryStatGate(modifier.type); });
    ...
    void SetLegacySummaryStats(Ride& ride, bool strong)
    {
        ...
        ride.maxNegativeVerticalG = strong ? MakeFixed16_2dp(-4, 00) : MakeFixed16_2dp(0, 00);
        ride.maxLateralG = strong ? MakeFixed16_2dp(1, 00) : MakeFixed16_2dp(4, 00);
        ride.numInversions = strong ? maximumByte : 0;
        ride.numHoles = strong ? maximumByte : 0;
        ride.shelteredLength = strong ? 0 : std::numeric_limits<int32_t>::max();
        if (strong)
    ...
    void PrepareAggregateRideWithStableSample(Ride& ride)
    {
        ...
        const auto sample = MakeRatingSample(
            100000 * RideRating::kRideRatingAccumulatorRawScale, 90000 * RideRating::kRideRatingAccumulatorRawScale,
            80000 * RideRating::kRideRatingAccumulatorRawScale, 100);
        RideAddRecentRatingSample(ride, sample);
    }

    void ExpectAggregateIgnoresLegacySummaryStats(Ride& ride)
    {
        PrepareAggregateRideWithStableSample(ride);
        SetLegacySummaryStats(ride, false);
        RideRating::UpdateRide(ride);
        const auto poorStatsRatings = ride.ratings;
        SetLegacySummaryStats(ride, true);
        RideRating::UpdateRide(ride);
        EXPECT_EQ(ride.ratings, poorStatsRatings);
    }

    std::string FormatRatings(const Ride& ride)
    {
        const auto& ratings = ride.ratings;
        return String::stdFormat(
            "%s: (%d, %d, %d)", ride.getRideTypeDescriptor().Name.data(), static_cast<int>(ratings.excitement),
    ...
    void TestRatings(const u8string& parkFile, uint16_t expectedRideCount)
    {
        ...
        const auto* updateValue = std::getenv("OPENRCT2_UPDATE_RIDE_RATINGS");
        const bool updateExpectedRatings = updateValue != nullptr && std::string_view(updateValue) == "1";
        ...
        std::vector<u8string> actualRatings;
        for (const auto& ride : RideManager(gameState))
            actualRatings.push_back(FormatRatings(ride));
        if (updateExpectedRatings)
        {
            std::string data;
            for (const auto& rating : actualRatings)
            {
                if (!data.empty())
                    data += '\n';
                data += rating;
            }
            File::WriteAllBytes(ratingsDataPath, data.data(), data.size());
            return;
        }
        ...
        for (size_t i = 0; i < actualRatings.size(); i++)
            ASSERT_EQ(actualRatings[i], expectedRatings[i]);
    }
};
...
{
    auto context = LoadPark("EverythingPark.park");
...
TEST_F(RideRatings, RecentAccumulatorAveragesLastTwentySamples)
{
    ...
    for (size_t i = 0; i < kRideRatingRecentSampleCount + 5; i++)
    {
        ...
        const auto sample = MakeRatingSample(value * 2, value * 4, value * 6, 1);
        RideAddRecentRatingSample(ride, sample);
    }
...
TEST_F(RideRatings, ActiveRiderTrainSampleCombinesCompletedVehicleLapsAtPublish)
{
    ...
    firstVehicleSample->transportDistance = 100;
    firstVehicleSample->transportShelteredDistance = 60;
    ...
    secondVehicleSample->transportDistance = 300;
    secondVehicleSample->transportShelteredDistance = 180;
    ...
    EXPECT_EQ(storedSample.transportDistance, 200);
    EXPECT_EQ(storedSample.transportShelteredDistance, 120);
...
{
    Ride ride{};
    InitialiseStableTransportRide(ride, 2);
...
{
    Ride ride{};
    InitialiseStableTransportRide(ride, 3);
    ride.stableStats.stations[0].SegmentLength = 400 << 16;
...
TEST_F(RideRatings, TransportServiceCacheBuildsEveryDirectedJourneyAndRefreshesOnInvalidation)
{
    auto context = InitialiseContext();
    ...
    const auto transportRideId = RideId::FromUnderlying(10);
    auto* ride = AllocateStableTransportRide(transportRideId, 3);
    ...
    for (size_t stationIndex = 0; stationIndex < ride->numStations; stationIndex++)
    {
        ...
        station.setStart({ static_cast<int32_t>(stationIndex * 10), static_cast<int32_t>(stationIndex * 20) });
        station.setEntrance({ static_cast<int32_t>(10 + stationIndex), 20, 2, 0 });
        station.setExit({ static_cast<int32_t>(30 + stationIndex), 40, 2, 2 });
    ...
    const auto service = RideGetTransportService(transportRideId);
    ...
    EXPECT_EQ(service.stations[0].entrance, ride->getStation(StationIndex::FromUnderlying(0)).getEntrance());
    EXPECT_EQ(service.stations[2].exit, ride->getStation(StationIndex::FromUnderlying(2)).getExit());
    ...
    for (size_t boardingIndex = 0; boardingIndex < ride->numStations; boardingIndex++)
    {
        ...
        for (size_t destinationIndex = 0; destinationIndex < ride->numStations; destinationIndex++)
        {
            ...
            const auto destination = StationIndex::FromUnderlying(static_cast<StationIndex::UnderlyingType>(destinationIndex));
    ...
    const auto afterLiveOverlays = RideGetTransportService(transportRideId);
    EXPECT_EQ(afterLiveOverlays.stations[0].entrance, initialEntrance);
    ...
    ride->getStation().setEntrance({ 50, 60, 3, 1 });
    ...
    EXPECT_EQ(afterMeasuredStats.stations[0].entrance, ride->getStation().getEntrance());
    ...
    RideClearRiderRatingSamples(*ride);
    qualitySample.transportComfort = 500 * qualitySample.transportDistance;
    RideAddRecentRatingSample(*ride, qualitySample);
    const auto afterQuality = RideGetTransportService(transportRideId);
...
TEST_F(RideRatings, TransportServiceCacheTracksAvailabilityDeletionReuseAndReset)
{
    ...
    const auto transportRideId = RideId::FromUnderlying(7);
    auto initialiseRide = [&](int32_t entranceX) {
        auto* ride = AllocateStableTransportRide(transportRideId, 2);
        if (ride == nullptr)
            return ride;
        ride->stableStats.stations[0].SegmentLength = 400 << 16;
        ride->stableStats.stations[1].SegmentLength = 600 << 16;
        ride->getStation(StationIndex::FromUnderlying(0)).setEntrance({ entranceX, 20, 2, 0 });
        ride->getStation(StationIndex::FromUnderlying(0)).setExit({ 30, 20, 2, 2 });
        ride->getStation(StationIndex::FromUnderlying(1)).setEntrance({ 40, 20, 2, 0 });
        ride->getStation(StationIndex::FromUnderlying(1)).setExit({ 50, 20, 2, 2 });
        return ride;
    };

    auto* ride = initialiseRide(10);
    ASSERT_NE(ride, nullptr);
    ASSERT_TRUE(RideGetTransportService(transportRideId).isAvailable());
    ...
    RideInvalidateTransportServiceCache(transportRideId);
    EXPECT_TRUE(RideGetTransportService(transportRideId).isAvailable());
    ...
    ride->status = RideStatus::open;
    RideInvalidateTransportServiceCache(transportRideId);
    const auto beforeDeletion = RideGetTransportService(transportRideId);
    ASSERT_TRUE(beforeDeletion.isAvailable());

    RideDelete(transportRideId);
    EXPECT_FALSE(RideGetTransportService(transportRideId).isAvailable());
    ...
    EXPECT_EQ(afterReuse.stations[0].entrance, replacement->getStation().getEntrance());
...
TEST_F(RideRatings, TransportServiceSpatialQueriesAreStableAcrossBoundariesMovesAndReuse)
{
    ...
    auto initialiseRide = [](RideId transportRideId, const TileCoordsXY& firstEntrance, const TileCoordsXY& secondEntrance) {
        auto* ride = AllocateStableTransportRide(transportRideId, 2);
        if (ride == nullptr)
            return ride;
        ride->stableStats.stations[0].SegmentLength = 400 << 16;
        ride->stableStats.stations[1].SegmentLength = 600 << 16;
        ride->getStation(StationIndex::FromUnderlying(0)).setEntrance({ firstEntrance, 2, 0 });
        ride->getStation(StationIndex::FromUnderlying(0)).setExit({ firstEntrance + TileCoordsXY{ 100, 0 }, 2, 2 });
        ride->getStation(StationIndex::FromUnderlying(1)).setEntrance({ secondEntrance, 2, 0 });
        ride->getStation(StationIndex::FromUnderlying(1)).setExit({ secondEntrance + TileCoordsXY{ 100, 0 }, 2, 2 });
        return ride;
    };
    ...
    EXPECT_FALSE(RideGetTransportService(highRideId).isAvailable());
    ...
    ASSERT_EQ(highRideBuffer.size(), 2u);
    EXPECT_EQ(highRideBuffer[0], (TransportRideServiceStationRef{ highRideId, StationIndex::FromUnderlying(0) }));
    EXPECT_EQ(highRideBuffer[1], (TransportRideServiceStationRef{ highRideId, StationIndex::FromUnderlying(1) }));
    ...
    ASSERT_EQ(orderedBuffer.size(), 3u);
    EXPECT_EQ(orderedBuffer[0], (TransportRideServiceStationRef{ lowRideId, StationIndex::FromUnderlying(0) }));
    EXPECT_EQ(orderedBuffer[1], (TransportRideServiceStationRef{ highRideId, StationIndex::FromUnderlying(0) }));
    EXPECT_EQ(orderedBuffer[2], (TransportRideServiceStationRef{ highRideId, StationIndex::FromUnderlying(1) }));
    ...
    ASSERT_EQ(exitBuffer.size(), 3u);
    EXPECT_EQ(exitBuffer[0].ride, lowRideId);

    std::vector<TransportRideServiceStationRef> allStations;
    RideCollectAllTransportServiceStations(TransportRideServiceEndpoint::boardingEntrance, allStations);
    ASSERT_EQ(allStations.size(), 4u);
    EXPECT_TRUE(std::is_sorted(allStations.begin(), allStations.end()));
    const auto warmedCapacity = allStations.capacity();
    RideCollectAllTransportServiceStations(TransportRideServiceEndpoint::boardingEntrance, allStations);
    EXPECT_EQ(allStations.size(), 4u);
    EXPECT_EQ(allStations.capacity(), warmedCapacity);
    ...
    highRide->getStation(StationIndex::FromUnderlying(0)).setEntrance({ 64, 64, 2, 0 });
    ...
    ASSERT_EQ(movedBuffer.size(), 1u);
    EXPECT_EQ(movedBuffer.front(), retainedCandidate);
    EXPECT_EQ(highRideBuffer.front(), retainedCandidate);
    ...
    EXPECT_TRUE(deletedBuffer.empty());
    ...
    ASSERT_EQ(reuseBuffer.size(), 2u);
    EXPECT_EQ(reuseBuffer[0].ride, lowRideId);
    EXPECT_EQ(reuseBuffer[1].ride, lowRideId);
}
...
TEST_F(RideRatings, TransportStationOvercrowdingRequiresFullQueueAndFullPlatform)
{
    ...
    ride.getStation().QueueFull = false;
    EXPECT_EQ(RideGetTransportStationPlatformCapacity(ride, stationIndex), 2);
    EXPECT_EQ(RideGetTransportStationPlatformOccupancy(ride, stationIndex), 2);
    EXPECT_FALSE(RideIsTransportStationOvercrowded(ride, stationIndex));
    ...
    EXPECT_FALSE(RideIsTransportStationOvercrowded(ride, stationIndex));
    head->num_peeps = 2;
    EXPECT_TRUE(RideIsTransportStationOvercrowded(ride, stationIndex));

    // Lift has no pre-queue adapter and therefore retains the conservative station-length capacity.
    ride.type = RIDE_TYPE_LIFT;
    head->num_peeps = 6;
    EXPECT_EQ(RideGetTransportStationPlatformCapacity(ride, stationIndex), 6);
    EXPECT_EQ(RideGetTransportStationPlatformOccupancy(ride, stationIndex), 6);
    EXPECT_TRUE(RideIsTransportStationOvercrowded(ride, stationIndex));
}

TEST_F(RideRatings, PlatformPreQueueAdaptersOptInScalarTransportAndCoasterStations)
{
    Ride ride{};
    ride.type = RIDE_TYPE_CHAIRLIFT;
    ...
    InitialiseRide(ride, RIDE_TYPE_WOODEN_ROLLER_COASTER, 1);
    ride.getStation().QueueFull = true;
    EXPECT_TRUE(RideSupportsStationPlatformPreQueue(ride));
    EXPECT_EQ(RideGetTransportStationPlatformCapacity(ride, StationIndex::FromUnderlying(0)), 0);
...
{
    Ride ride{};
    InitialiseRide(ride, RIDE_TYPE_WOODEN_ROLLER_COASTER, 3);

    auto first = MakeRatingSample(
        900'000, 300'000, 200'000, 80, StationIndex::FromUnderlying(0), StationIndex::FromUnderlying(2));
    ...
    EXPECT_EQ(
        ride.ratings.excitement, std::min({ firstRatings.excitement, secondRatings.excitement, thirdLeg->ratings.excitement }));
    EXPECT_EQ(
        ride.ratings.intensity, std::max({ firstRatings.intensity, secondRatings.intensity, thirdLeg->ratings.intensity }));
    EXPECT_EQ(ride.ratings.nausea, std::max({ firstRatings.nausea, secondRatings.nausea, thirdLeg->ratings.nausea }));
...
TEST_F(RideRatings, SingleStationSamplesRetainRideWideHistoryWithoutLegAllocation)
{
    ...
    const auto sample = MakeRatingSample(500'000, 400'000, 300'000, 40);
...
TEST_F(RideRatings, ThroughRiderCarsPublishOneLegThenStartCleanForTheNextLeg)
{
    ...
    EXPECT_TRUE(RideRating::RecordActiveRiderSamples(ride, sampleEntities));
    const auto* completedLeg = RideGetRatingLeg(ride, StationIndex::FromUnderlying(0), StationIndex::FromUnderlying(2));
    ...
    EXPECT_TRUE(RideRating::RecordActiveRiderSamples(ride, sampleEntities));
    EXPECT_NE(RideGetRatingLeg(ride, StationIndex::FromUnderlying(2), StationIndex::FromUnderlying(1)), nullptr);
}
...
TEST_F(RideRatings, DirectedLegStorageAllowsTwoDestinationsFromOneShuttleStation)
{
    ...
    auto towardStart = MakeRatingSample(
        300'000, 700'000, 600'000, 20, StationIndex::FromUnderlying(1), StationIndex::FromUnderlying(0));
...
TEST_F(RideRatings, RatingLegDisplaySelectionReachesLaterEdgesInStableEndpointOrder)
{
    ...
    const auto addLeg = [&ride](uint8_t origin, uint8_t destination) {
        auto sample = MakeRatingSample(
            100'000 + origin * 10'000 + destination, 100'000, 100'000, 20, StationIndex::FromUnderlying(origin),
            StationIndex::FromUnderlying(destination));
        RideRating::RecordRiderSample(ride, sample);
    };
    addLeg(2, 1);
    addLeg(0, 2);
    ...
    const std::array expected = {
        std::pair{ uint8_t{ 0 }, uint8_t{ 1 } }, std::pair{ uint8_t{ 0 }, uint8_t{ 2 } },
        std::pair{ uint8_t{ 1 }, uint8_t{ 0 } }, std::pair{ uint8_t{ 1 }, uint8_t{ 2 } },
...
TEST_F(RideRatings, IncompleteDirectedCoveragePreservesNullCompatibility)
{
    ...
    auto sample = MakeRatingSample(
        200'000, 100'000, 50'000, 20, StationIndex::FromUnderlying(0), StationIndex::FromUnderlying(2));
    RideRating::RecordRiderSample(ride, sample);
...
{
    Ride ride{};
    InitialiseRide(ride, RIDE_TYPE_MONORAIL, 3);
    ride.stableStats.valid = true;
    ...
    AddMeasuredTransportLeg(ride, 0, 2, 4'000, 40, 0x80000, 0x90000);
    AddMeasuredTransportLeg(ride, 2, 1, 5'000, 60, 0xA0000, 0xB0000);
    AddMeasuredTransportLeg(ride, 1, 0, 6'000, 80, 0x70000, 0x80000);
...
TEST_F(RideRatings, TransportJourneyPrefersMoreShelteredEqualTimeAndFarePath)
{
    Ride ride{};
    InitialiseRide(ride, RIDE_TYPE_MONORAIL, 4);

    AddMeasuredTransportLeg(ride, 0, 1, 1'000, 40, 0x80000, 0x80000);
    AddMeasuredTransportLeg(ride, 1, 3, 1'000, 40, 0x80000, 0x80000);
    AddMeasuredTransportLeg(ride, 0, 2, 1'000, 40, 0x80000, 0x80000, true);
    AddMeasuredTransportLeg(ride, 2, 3, 1'000, 40, 0x80000, 0x80000, true);

    const auto journey = RideGetTransportJourney(ride, StationIndex::FromUnderlying(0), StationIndex::FromUnderlying(3));
    ASSERT_EQ(journey.segmentCount, 2);
    EXPECT_EQ(journey.shelteredTravelTimeMilliseconds, journey.travelTimeMilliseconds);
}
...
TEST_F(RideRatings, TransportJourneyKeepsBothMeasuredShuttleDepartures)
{
    ...
    AddMeasuredTransportLeg(ride, 1, 0, 2'000);
    AddMeasuredTransportLeg(ride, 1, 2, 3'000);
    AddMeasuredTransportLeg(ride, 0, 1, 2'000);
    AddMeasuredTransportLeg(ride, 2, 1, 3'000);
    ...
    const auto* measuredStartLeg = RideGetRatingLeg(ride, StationIndex::FromUnderlying(1), StationIndex::FromUnderlying(0));
...
TEST_F(RideRatings, SampledRatingProfilesApplyInitialRideTypeMultipliers)
{
    ...
    ExpectTickScore(kartSpeed, { defaultSpeed.excitement * 3, defaultSpeed.intensity * 3, defaultSpeed.nausea * 3 });
}
...
TEST_F(RideRatings, VehicleGForceSpeedContextPreparesTrainWideCouplingExactly)
{
    constexpr int64_t denominator = static_cast<int64_t>(RideRating::kVehicleRatingBaselineSpeed)
        * kSampledRideRatingProfileScale;
    const auto halfSpeed = RideRating::PrepareVehicleGForceSpeedContext(45, 1000);
    const auto baseline = RideRating::PrepareVehicleGForceSpeedContext(90, 1000);
    const auto doubleSpeed = RideRating::PrepareVehicleGForceSpeedContext(180, 1000);
    const auto clampedInputs = RideRating::PrepareVehicleGForceSpeedContext(-20, -5);
    const auto stoppedCoupled = RideRating::PrepareVehicleGForceSpeedContext(0, 1000);

    EXPECT_EQ(halfSpeed.normalisedSpeed, 45);
    EXPECT_EQ(halfSpeed.couplingNumerator, denominator / 2);
    EXPECT_EQ(baseline.couplingNumerator, denominator);
    EXPECT_EQ(doubleSpeed.couplingNumerator, denominator * 2);
    EXPECT_EQ(clampedInputs.normalisedSpeed, 0);
    EXPECT_EQ(clampedInputs.couplingNumerator, denominator);
    EXPECT_EQ(stoppedCoupled.couplingNumerator, 0);

    SampledRideRatingProfile profile{};
    profile.SpeedGCoupling = 1000;
    const auto direct = RideRating::ScoreGForcesForVehicleTick(100, 100, 0, 45, profile);
    const auto prepared = RideRating::ScoreGForcesForVehicleTick(100, 100, 0, profile, halfSpeed);
    ExpectTickScore(prepared, direct);
}

TEST_F(RideRatings, VehicleGForceScoreMemoMatchesDirectScoringForHitsMissesAndSignedForces)
{
    SampledRideRatingProfile profile{};
    profile.VerticalG = 1700;
    profile.LateralG = 800;
    profile.LongitudinalG = 1300;
    profile.Airtime = 2100;
    profile.SpeedGCoupling = 750;
    const auto speedContext = RideRating::PrepareVehicleGForceSpeedContext(137, profile.SpeedGCoupling);
    RideRating::VehicleGForceScoreMemo memo{ profile, speedContext };

    const auto expectDirectScore = [&](int32_t verticalG, int32_t lateralG, int32_t longitudinalG) {
        const auto direct = RideRating::ScoreGForcesForVehicleTick(verticalG, lateralG, longitudinalG, profile, speedContext);
        const auto memoized = memo.Get(verticalG, lateralG, longitudinalG);
        ExpectTickScore(memoized, direct);
    };

    expectDirectScore(100, 0, 0);
    expectDirectScore(100, 0, 0); // Exact-key hit.
    expectDirectScore(100, 0, 100);
    expectDirectScore(100, 0, -100); // Signed longitudinal G is part of the key.
    expectDirectScore(-100, 310, -250);
    expectDirectScore(100, 0, 0); // Recalculation after one-entry replacement.
}
...
TEST_F(RideRatings, RideEntryMultipliersApplyToRawAggregateBeforeGeometricFinalRating)
{
    ...
    ExpectTickScore(score, { 160, 256, 768 });
}
...
TEST_F(RideRatings, SceneryVisibilityMultiplierUsesRideTypePolicy)
{
    ...
    struct TestCase
    {
        ...
        std::pair<int32_t, int32_t> expected;
    };
    constexpr TestCase testCases[] = {
        { RIDE_TYPE_CIRCUS, { 0, 1 } },  { RIDE_TYPE_HAUNTED_HOUSE, { 1, 2 } },     { RIDE_TYPE_MAZE, { 1, 1 } },
        { RIDE_TYPE_DODGEMS, { 1, 4 } }, { RIDE_TYPE_OBSERVATION_TOWER, { 1, 1 } },
    };
    for (const auto& [rideType, expected] : testCases)
    {
        SCOPED_TRACE(static_cast<int32_t>(rideType));
        ride.type = rideType;
        EXPECT_EQ(RideRating::GetSceneryVisibilityMultiplier(ride), expected);
    }
}
...
TEST_F(RideRatings, VehicleLocalContextPreparedScoreMatchesDirectAcrossSpeedModeProfileAndClear)
{
    const RideRating::LocalContextScore contextScore = {
        .excitement = 31,
        .intensity = 17,
        ...
        .scenery = 6,
        .pathProximity = 5,
        .foreignTrackProximity = 4,
        .verticalInteraction = 3,
        .pathBridge = 2,
        .pathNearMiss = 2,
        .pathLoop = 1,
        .trackVerticalInteraction = 2,
        .ownTrackVerticalInteraction = 1,
        .trackHeightExposure = 3,
    };
    RideRating::VehicleLocalContextCache runtimeCache{};
    runtimeCache.environment.context = contextScore;
...
TEST_F(RideRatings, VehicleLocalContextRuntimeCacheTracksNearbyMapChanges)
{
    ...
    const auto sharedCacheHit = RideRating::GetVehicleRatingEnvironment(
        origin, rideId, TrackElemType::flatTrack1x4A, 0, secondRuntimeCache);
    EXPECT_EQ(sharedCacheHit.context, before.context);
    EXPECT_EQ(sharedCacheHit.isSheltered, before.isSheltered);
    const auto sharedHitScore = RideRating::ScoreCachedLocalContextForVehicleTick(secondRuntimeCache, 90, 1000, false);
    ExpectTickScore(sharedHitScore, expectedBeforeScore);
    ...
    PlacePath(cacheOriginTile, groundZ + (4 * kCoordsZStep), groundZ + (5 * kCoordsZStep));
...
TEST_F(RideRatings, LocalContextRangeUsesHeightAboveLocalGround)
{
    ...
    auto* sceneryElement = InsertTileElement<SmallSceneryElement>(
        { sceneryTile.toCoordsXY(), plateauZ }, 0,
        [=](SmallSceneryElement& sceneryElement) { sceneryElement.setClearanceZ(plateauZ + (4 * kCoordsZStep)); });
    ASSERT_NE(sceneryElement, nullptr);
...
{
    auto fixture = PrepareCachedSceneryContext();
...
TEST_F(RideRatings, ClearLocalContextCacheMakesStoredPayloadsUnreachable)
{
    auto fixture = PrepareCachedSceneryContext();
    ASSERT_NE(fixture.scenerySurface, nullptr);
    const auto before = RideRating::GetLocalContextScore(fixture.origin, rideId);
...
TEST_F(RideRatings, ViewportOnlyInvalidationKeepsStoredLocalContextPayloads)
{
    auto fixture = PrepareCachedSceneryContext();
    ASSERT_NE(fixture.scenerySurface, nullptr);
    const auto before = RideRating::GetLocalContextScore(fixture.origin, rideId);
...
TEST_F(RideRatings, LocalContextOneExposedSideScoresLessThanTwo)
{
    ...
    const auto twoSideScore = GetVehicleLocalContext(twoSideOrigin, rideId, TrackElemType::flatTrack1x4A, trackDirection);
    const auto oneSideScore = GetVehicleLocalContext(oneSideOrigin, rideId, TrackElemType::flatTrack1x4A, trackDirection);
...
TEST_F(RideRatings, LocalContextSameRideVerticalTrackScoresBelowForeignTrack)
{
    ...
    const auto foreignOrigin = PlaceVehicleTrackWithTerrain(foreignTile, trackZ, trackZ, trackZ);
    PlaceFlatTrack(
        foreignTile, upperTrackZ, upperTrackZ + (2 * kCoordsZStep), foreignRideId, TrackElemType::flatTrack1x4A,
        trackDirection);

    const auto ownOrigin = PlaceVehicleTrackWithTerrain(ownTile, trackZ, trackZ, trackZ);
    PlaceFlatTrack(
        ownTile, upperTrackZ, upperTrackZ + (2 * kCoordsZStep), rideId, TrackElemType::flatTrack1x4A, trackDirection);

    const auto foreignScore = GetVehicleLocalContext(foreignOrigin, rideId, TrackElemType::flatTrack1x4A, trackDirection);
    const auto ownScore = GetVehicleLocalContext(ownOrigin, rideId, TrackElemType::flatTrack1x4A, trackDirection);
...
TEST_F(RideRatings, LocalContextOneWideBridgeAdjacentToFlatVehicleAddsExcitementOnly)
{
    ...
    EXPECT_EQ(score.pathLoop, 0);
    EXPECT_EQ(score.trackVerticalInteraction, 0);
    ExpectTickScore(score, { score.pathBridge, 0, 0 });
}
...
TEST_F(RideRatings, LocalContextPathBelowVerticalLoopAddsLoopThrill)
{
    ...
    constexpr int32_t trackZ = 18 * kCoordsZStep;
    const auto origin = PlaceVehicleTrackWithTerrain(originTile, trackZ, trackZ, trackZ, TrackElemType::leftVerticalLoop);
    PlacePath(originTile, trackZ - (3 * kCoordsZStep), trackZ - (2 * kCoordsZStep));
...
TEST_F(RideRatings, FixedRideContextOriginUsesRideSpecificEyeHeight)
{
    ...
    Ride groundRide{};
    InitialiseFixedRide(groundRide, RideId::FromUnderlying(1), RIDE_TYPE_MERRY_GO_ROUND, stationTile, baseZ);

    Ride ferrisRide{};
    InitialiseFixedRide(ferrisRide, RideId::FromUnderlying(2), RIDE_TYPE_FERRIS_WHEEL, stationTile, baseZ);

    Ride indoorRide{};
    InitialiseFixedRide(indoorRide, RideId::FromUnderlying(3), RIDE_TYPE_HAUNTED_HOUSE, stationTile, baseZ);
...
TEST_F(RideRatings, LocalContextBridgeAboveMazeAddsExcitementToAdjacentTilesOnly)
{
    ...
    ExpectTickScore(adjacentScore, { adjacentScore.pathBridge, 0, 0 });
...
{
    auto context = LoadPark("bpb.sv6");

    auto* target = FindNormalRide();
...
TEST_F(RideRatings, CompletedAggregateSampleMarksRideTestedAndPublishesRatings)
{
    ...
    auto* target = FindNormalRide(true);
    ASSERT_NE(target, nullptr);
    ...
    const auto sample = MakeRatingSample(
        100000 * RideRating::kRideRatingAccumulatorRawScale, 80000 * RideRating::kRideRatingAccumulatorRawScale,
        60000 * RideRating::kRideRatingAccumulatorRawScale, 100);
...
TEST_F(RideRatings, AggregateVehicleRatingsIgnoreLegacySummaryStatGates)
{
    ...
    auto* target = FindNormalRide(true);
    ASSERT_NE(target, nullptr);
    ExpectAggregateIgnoresLegacySummaryStats(*target);
}
