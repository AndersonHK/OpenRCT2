...
namespace
{
    ...
    json_t Read(const std::filesystem::path& path)
    {
        ...
        json_t value; file >> value; return value;
    }
    ...
    {
        Terrain::RetainedTerrainSnapshot snapshot; snapshot.worldEpoch = 1;
        const auto recipe = NonuniformTerrainFixture::MakeTiles();
        for (uint32_t index = 0; index < Terrain::kRetainedChunkCount; index++)
        {
            auto chunk = std::make_shared<Terrain::RetainedTileChunk>(); chunk->revision = index + 1;
        ...
        auto table = std::make_shared<Terrain::RetainedMaterialTable>(); table->revision = 1;
        for (const auto& value : materials)
        {
            Terrain::RetainedMaterial material{};
            material.imageBase = value.at("imageBase"); material.imageCount = value.at("imageCount"); material.kind = value.at("kind");
            material.selectors.entries = value.at("selectors").get<decltype(material.selectors.entries)>();
            table->records.push_back(material);
        }
        snapshot.materials = std::move(table); return snapshot;
    }
    struct Asset { json_t metadata; uint32_t index; int32_t x, y; std::vector<std::byte> pixels; };
    struct Corpus
    {
        ...
        explicit Corpus(const std::filesystem::path& folder)
        {
            ...
            Require(manifest.at("schema") == 1 && manifest.at("cases").size() == 32, "Expected 32 frozen terrain camera/policy cases");
            ...
            for (const auto& name : manifest.at("cases"))
            {
                ...
                Require(relative == relative.filename() && relative.extension() == ".json", "Corpus case must be a contained filename");
                auto value = Read(folder / relative);
                Require(value.at("schema") == 2 && value.at("landscapeSmoothing") == false && value.at("clearIndex") == 0,
                ...
                Require(combinations.insert(rotation + 4 * zoom + 8 * transparent + 16 * stable).second, "Duplicate corpus camera policy");
                Require(!value.at("columns").empty(), "Empty terrain trace");
                if (cases.empty()) snapshot = Snapshot(value.at("materials"));
                else Require(value.at("materials") == cases.front().at("materials"), "Corpus material generation changed");
                ...
                for (const auto& metadata : value.at("observedSpriteMetadata"))
                {
                    ...
                    if (assets.contains(image)) Require(assets.at(image).metadata == metadata, "Corpus sprite generation changed");
                    else assets.emplace(image, Asset{ metadata, 0, 0, 0, {} });
                }
                cases.push_back(std::move(value));
            }
            Require(combinations.size() == 32 && !originals.empty(), "Incomplete terrain corpus coverage");
            int32_t x = 0, y = 0, rowHeight = 0; uint32_t index = 0;
            for (auto& [image, asset] : assets)
            {
                ...
                Require(asset.metadata.at("coveredZeroPixels") == 0, "Covered-zero asset needs an explicit rendering path before admission");
                if (x + width > 2048) { x = 0; y += rowHeight; rowHeight = 0; }
                Require(y + height <= 2048, "Terrain diagnostic atlas capacity exceeded");
                asset.index = index++; asset.x = x; asset.y = y;
                x += width; rowHeight = std::max(rowHeight, height);
            ...
            for (const auto image : originals)
            {
                ...
                metadata.imageIndex = image; metadata.width = original.at("width"); metadata.height = original.at("height");
                metadata.xOffset = original.at("xOffset"); metadata.yOffset = original.at("yOffset");
                ...
                for (int32_t zoom = 0; zoom < 2; zoom++)
                {
                    ...
                    metadata.variants[zoom] = { variant.at("width"), variant.at("height"), variant.at("xOffset"), variant.at("yOffset"),
                        asset.index, (variant.at("flags").get<uint32_t>() & 4u) != 0 ? 1u : 0u, linked ? 0 : zoom, linked ? 1 : 0 };
                }
    ...
    protected:
        ...
        void SetUp() override
        {
            ...
            if (!corpusPath || !emission || !columns || !shaders)
            {
                ...
                if (required && std::string_view(required) == "1") FAIL() << "Terrain drawing requires pinned corpus, emission and column shaders";
            ...
            if (testName == "ZeroPublicationOnInvalidInputsAndCapacityOverflow") artifacts /= "guards";
            if (testName == "ImmutableGenerationsAbandonAndSlotReusePreserveFrozenRaster") artifacts /= "generations";
            std::filesystem::create_directories(artifacts);
        }
        void TearDown() override
        {
            slots.reset(); pipeline.Dispose(); resources.Dispose(); device.reset();
        }
        void UploadAssets(const Vulkan::SubmissionToken& token)
        {
            ...
            for (size_t i = 0; i < 256 * 256; i++) remap.data[i] = static_cast<std::byte>(i % 256);
            ...
            for (const auto& [image, asset] : corpus->assets)
            {
                ...
                std::memcpy(upload.data, asset.pixels.data(), asset.pixels.size()); upload.RecordHostWrite();
                const int32_t width = asset.metadata.at("width"), height = asset.metadata.at("height");
                resources.RecordAtlasUpload(token.commandBuffer, upload, 0, { asset.x, asset.y, asset.x + width, asset.y + height }, width);
                ...
                std::memcpy(descriptorUpload.data, &descriptor, sizeof(descriptor)); descriptorUpload.RecordHostWrite();
        ...
        template<typename T> static T Load(const std::byte* bytes, size_t index)
        { T value{}; std::memcpy(&value, bytes + index * sizeof(T), sizeof(T)); return value; }
        ...
        void Run(size_t sample) { Run(sample, RunOptions{}); }
        void Run(size_t sample, const RunOptions& options)
        {
            ...
            Terrain::DrawCamera camera{ target[0], target[1], target[2], target[3], 0, 0,
                reference.at("rotation"), reference.at("zoom"), reference.at("transparent").get<bool>() ? 1u : 0u,
                reference.at("stableSort").get<bool>() ? 1u : 0u, 1 };
            camera.parentCapacity = options.parentCapacity; camera.commandCapacity = options.commandCapacity;
            OpenRCT2::Drawing::RenderUploadTelemetry telemetry;
            auto token = slots->Begin(options.frameIndex, true, &telemetry); Require(token.has_value(), "No terrain drawing submission slot");
            if (sample == 0) UploadAssets(*token);
            ...
            auto readback = token->upload->Allocate(bytes, 16); Require(static_cast<bool>(readback), "No terrain drawing readback allocation");
            const std::array<std::pair<const Vulkan::Buffer*, size_t>, 3> buffers = {{
                { &pipeline.GetParentBuffer(), parentOffset }, { &pipeline.GetColumnBuffer(), columnOffset },
                { &pipeline.GetCommandBuffer(), commandOffset } }};
            const VkMemoryBarrier reuse{ .sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER,
                .srcAccessMask = VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT, .dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT };
            ...
            vkCmdPipelineBarrier(token->commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                0, 1, &poison, 0, nullptr, 0, nullptr);
            for (const auto& [buffer, offset] : buffers)
            {
            ...
            Vulkan::RecordImageBarrier(token->commandBuffer, resources.GetIndexedCanvas(options.frameIndex).GetImage(),
            ...
            const VkBufferImageCopy imageCopy{ .bufferOffset = readback.offset,
                .imageSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 }, .imageExtent = { 960, 640, 1 } };
            vkCmdCopyImageToBuffer(token->commandBuffer, resources.GetIndexedCanvas(options.frameIndex).GetImage(),
            ...
            slots->Submit(*token); Require(slots->Wait(*token, UINT64_MAX), "Terrain drawing submission did not complete");
            ...
            EXPECT_TRUE(std::all_of(completedReadback.data() + beforeOffset, completedReadback.data() + bytes,
            ...
            { palette[i * 4] = palette[i * 4 + 1] = palette[i * 4 + 2] = static_cast<std::byte>(i); palette[i * 4 + 3] = std::byte{255}; }
            ...
            const auto wantedUpload = options.expectedUpload != UINT64_MAX ? options.expectedUpload : sample == 0 ? initialBytes : uint64_t{0};
            EXPECT_EQ(uploaded, wantedUpload);
            json_t metadata = { { "rotation", camera.rotation }, { "zoom", camera.zoom }, { "transparent", camera.transparent },
                { "stableSort", camera.stableSort }, { "uploadedWorldBytes", uploaded }, { "runtimeAdmission", false },
                { "expectedUploadBytes", wantedUpload }, { "expectedGpuError", options.expectedError },
                { "parentCapacity", camera.parentCapacity }, { "commandCapacity", camera.commandCapacity },
                { "worldEpoch", snapshot.worldEpoch }, { "materialRevision", snapshot.materials->revision },
                { "spriteRevision", sprites.revision },
                { "frameIndex", options.frameIndex }, { "submissionGeneration", token->generation },
            ...
            EXPECT_EQ(VulkanParitySupport::CompareAndReport(artifacts, std::to_string(sample), "indexed",
                expected, actual, 1, palette, metadata, {960,640}), 0u);
            uint32_t maximumError = 0;
            for (size_t columnIndex = 0; columnIndex < Terrain::kDrawMaximumColumns; columnIndex++)
            {
                ...
                {
                    EXPECT_TRUE(std::all_of(completedReadback.data() + columnOffset + columnIndex * sizeof(column),
                        completedReadback.data() + columnOffset + (columnIndex + 1) * sizeof(column), [](auto byte) { return byte == std::byte{0xa5}; }));
                }
                ...
                ASSERT_LE(parentCount, camera.parentCapacity); ASSERT_LE(commandCount, camera.commandCapacity);
                const auto parentStart = parentOffset + (columnIndex * Terrain::kDrawColumnCapacity + parentCount) * sizeof(Terrain::DrawParent);
                const auto parentEnd = parentOffset + (columnIndex + 1) * Terrain::kDrawColumnCapacity * sizeof(Terrain::DrawParent);
                const auto commandStart = commandOffset + (columnIndex * Terrain::kDrawColumnCapacity + commandCount) * sizeof(Gpu::SpriteCommand);
                const auto commandEnd = commandOffset + (columnIndex + 1) * Terrain::kDrawColumnCapacity * sizeof(Gpu::SpriteCommand);
                EXPECT_TRUE(std::all_of(completedReadback.data() + parentStart, completedReadback.data() + parentEnd, [](auto byte) { return byte == std::byte{0xa5}; }));
                EXPECT_TRUE(std::all_of(completedReadback.data() + commandStart, completedReadback.data() + commandEnd, [](auto byte) { return byte == std::byte{0xa5}; }));
                if (columnIndex >= reference.at("columns").size()) continue;
                ...
                if (options.expectedError != 0) continue;
                const auto& frozen = reference.at("columns").at(columnIndex);
                EXPECT_EQ(column.error, 0u); EXPECT_EQ(column.firstInstance, 0u); EXPECT_EQ(column.vertexCount, 4u);
                ...
                {
                    ASSERT_GE(node, 0); ASSERT_LT(static_cast<uint32_t>(node), column.parentCount);
                    ASSERT_TRUE(visited.insert(node).second);
                    const auto parent = Load<Terrain::DrawParent>(completedReadback.data() + parentOffset,
                    ...
                    EXPECT_EQ((std::array{parent.x,parent.y,parent.z,parent.xEnd,parent.yEnd,parent.zEnd}),
                        (original.at("bounds").get<std::array<int32_t,6>>()));
                    EXPECT_EQ((std::array{parent.screenX,parent.screenY}), (original.at("screen").get<std::array<int32_t,2>>()));
                    EXPECT_EQ(parent.attachedCount, original.at("attached").size());
                    node = parent.next;
                }
                EXPECT_EQ(node, -1); EXPECT_EQ(visited.size(), column.parentCount);
            }
...
TEST_F(VulkanRetainedTerrainDrawTest, FrozenColumnsAndActualIndexedPixelsAcrossAllCameraPolicies)
{
    ...
    { SCOPED_TRACE(sample); Run(sample); ASSERT_FALSE(HasFatalFailure()); }
...
{
    Run(0); ASSERT_FALSE(HasFatalFailure());
    RunOptions options; options.referenceCase = 0;
    options.parentCapacity = 0; options.expectedError = 2;
    Run(1, options); ASSERT_FALSE(HasFatalFailure());
    options.parentCapacity = Terrain::kDrawColumnCapacity; options.commandCapacity = 0;
    Run(2, options); ASSERT_FALSE(HasFatalFailure());
    options.parentCapacity = 1; options.commandCapacity = Terrain::kDrawColumnCapacity;
    Run(3, options); ASSERT_FALSE(HasFatalFailure());
    options.parentCapacity = Terrain::kDrawColumnCapacity; options.commandCapacity = 1;
    Run(4, options); ASSERT_FALSE(HasFatalFailure());
    auto invalid = corpus->snapshot;
    auto chunk = std::make_shared<Terrain::RetainedTileChunk>(*invalid.chunks[0]);
    chunk->revision = 100; chunk->records[33].baseZ = 15; invalid.chunks[0] = std::move(chunk);
    options.commandCapacity = Terrain::kDrawColumnCapacity; options.expectedError = 1;
    options.snapshot = &invalid; options.expectedUpload = sizeof(Terrain::RetainedTileChunk::records);
    Run(5, options); ASSERT_FALSE(HasFatalFailure());
    options.snapshot = nullptr; options.expectedError = 0;
    Run(6, options); ASSERT_FALSE(HasFatalFailure());
    auto missing = corpus->sprites; missing.revision = 2; missing.records.resize(1);
    options.sprites = &missing; options.expectedError = 3; options.expectedUpload = sizeof(Terrain::DrawSpriteMetadata);
    Run(7, options); ASSERT_FALSE(HasFatalFailure());
    options.sprites = nullptr; options.expectedError = 0;
    options.expectedUpload = corpus->sprites.records.size() * sizeof(Terrain::DrawSpriteMetadata);
    Run(8, options); ASSERT_FALSE(HasFatalFailure());
    ...
    EXPECT_THROW(pipeline.Record(*token, corpus->snapshot, corpus->sprites, camera,
        static_cast<uint32_t>(corpus->assets.size())), std::invalid_argument);
    camera.parentCapacity = Terrain::kDrawColumnCapacity; camera.commandCapacity = Terrain::kDrawColumnCapacity + 1;
    EXPECT_THROW(pipeline.Record(*token, corpus->snapshot, corpus->sprites, camera,
        static_cast<uint32_t>(corpus->assets.size())), std::invalid_argument);
    slots->Abandon(*token); pipeline.DiscardPendingUploads();
    options.expectedUpload = Terrain::kRetainedTileCount * sizeof(Terrain::RetainedTile)
        + 4 * sizeof(Terrain::RetainedMaterial) + corpus->sprites.records.size() * sizeof(Terrain::DrawSpriteMetadata);
    Run(9, options); ASSERT_FALSE(HasFatalFailure());
    RecordProperty("terrainDrawSamples", 10); RecordProperty("cpuRejections", 2);
}
TEST_F(VulkanRetainedTerrainDrawTest, ImmutableGenerationsAbandonAndSlotReusePreserveFrozenRaster)
{
    ...
    RunOptions options; options.referenceCase = 0; options.snapshot = &snapshot; options.sprites = &sprites;
    auto chunk = std::make_shared<Terrain::RetainedTileChunk>(*snapshot.chunks[0]);
    chunk->revision = 100; snapshot.chunks[0] = std::move(chunk);
    options.expectedUpload = sizeof(Terrain::RetainedTileChunk::records); options.frameIndex = 1;
    Run(1, options); ASSERT_FALSE(HasFatalFailure());
    auto materials = std::make_shared<Terrain::RetainedMaterialTable>(*snapshot.materials);
    materials->revision = 2; snapshot.materials = std::move(materials);
    options.expectedUpload = 4 * sizeof(Terrain::RetainedMaterial); options.frameIndex = 0;
    Run(2, options); ASSERT_FALSE(HasFatalFailure());
    sprites.revision = 2; options.expectedUpload = sprites.records.size() * sizeof(Terrain::DrawSpriteMetadata);
    options.frameIndex = 1;
    Run(3, options); ASSERT_FALSE(HasFatalFailure());
    snapshot.worldEpoch = 2;
    const uint64_t allBytes = Terrain::kRetainedTileCount * sizeof(Terrain::RetainedTile)
        + 4 * sizeof(Terrain::RetainedMaterial) + sprites.records.size() * sizeof(Terrain::DrawSpriteMetadata);
    options.expectedUpload = allBytes; options.frameIndex = 0;
    Run(4, options); ASSERT_FALSE(HasFatalFailure());
    auto token = slots->Begin(1, true); ASSERT_TRUE(token.has_value());
    ...
    pipeline.Record(*token, snapshot, sprites, camera, static_cast<uint32_t>(corpus->assets.size()));
    slots->Abandon(*token); resources.DiscardFrameLayouts(1); pipeline.DiscardPendingUploads();
    options.frameIndex = 1;
    Run(5, options); ASSERT_FALSE(HasFatalFailure());
    EXPECT_THROW(slots->Submit(*token), std::logic_error);
    options.expectedUpload = 0; options.frameIndex = 0; options.referenceCase = 31;
    Run(6, options); ASSERT_FALSE(HasFatalFailure());
    options.frameIndex = 1; options.referenceCase = 0;
    Run(7, options); ASSERT_FALSE(HasFatalFailure());
    RecordProperty("terrainDrawSamples", 8); RecordProperty("abandonedRecordings", 1);
}
#endif
