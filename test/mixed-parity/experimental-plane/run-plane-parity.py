...
def load_helper(name, filename):
    spec = importlib.util.spec_from_file_location(name, (next(p for p in Path(__file__).resolve().parents if (p / 'scripts/rendering/run-mixed-parity.py').is_file() and (p / 'src/openrct2').is_dir())) / 'scripts/rendering' / filename)
...
def mixed_inputs(root, summary_path, probe_path, build):
    ...
    capture.require(probe.get('status') == 'pass' and probe.get('inputsUnchanged') is True
                    and probe.get('model') == 'plane-envelope-v1'
                    and set(probe.get('inputSha256', {})) == set(plane_builder.INPUTS), 'Qualified complete mixed emitter receipt required')
    pin(Path(plane_builder.__file__), probe['builderSha256'])
...
def install_inputs(output, directory, shader, proof, pinned):
    ...
    raster = output / 'mixed-plane-shaders'
    raster.mkdir()
    for name, digest in proof['rasterShaderSha256'].items():
        ...
        pinned[destination] = digest
    return {'OPENRCT2_MIXED_DEPTH_MODEL': 'plane-envelope-v1', 'OPENRCT2_MIXED_CORPUS': str(isolated), 'OPENRCT2_MIXED_EMISSION_SPV': str(emitter),
            ...
            for layer, channels in (('indexed', 1), ('rgba', 4)):
                try:
                    ...
                    capture.require(report['acceptedExceptions'] == []
                                    and report['depthHypothesis'] == 'plane-envelope-v1'
...
def main():
    ...
    parser.add_argument('--plane-probe-receipt', type=Path, required=True)
    ...
    try:
        ...
        directory, shader, extra, proof = mixed_inputs(root, args.mixed_corpus_summary, args.plane_probe_receipt, build)
        pinned.update(extra)
        diagnostic_env = install_inputs(output, directory, shader, proof, pinned)
        shader_directory = output / 'mixed-plane-shaders'
        ...
        canonical = {name: value for name, value in summary['reports'].items() if '-legacy-' in name}
