...
## Structural local-depth correction after build144 (qualification pending)

The saved Everything Park support closeup `obj/vulkan-parity/everything141-manual-three/flat-supports.png` shows metal shafts/braces crossing their own rail ribbons. The all-rotation glass closeups in `everything144-matched-art-01/manual-glass-review` retain coherent front/back panes; missing people are a separate layer, not evidence of a glass depth defect. Matched-art143 path checkers are the correct RCT1AA surface art. The earlier software comparison lacked CSG and must not be used to diagnose those tiles as native support corruption. Ordinary banner lettering absence is separately traced to packed banner metadata, not this depth fix.

The former two-ULP bias allowed local children to pass unrelated integer anchors. The new contract keeps `D = rotatedX + rotatedY + Z`, assigns the exact priority offset `4*(D+131072)`, and uses **one D32 ULP per local layer, 0..15**. The admitted integer range is `[-131072,131071]`. Adjacent anchors have at least sixteen ULPs of separation throughout the positive world depth interval. The last local layer therefore remains behind the next integer anchor, and the whole reserved interval remains behind subsequent UI. No texel, screen position, bounding-box maximum or image-family rank enters the scalar. Filter collection already consumes the resulting hardware depth directly.

Parent capture now rejects out-of-range anchors and layers explicitly. Selected-car compatibility packets allow up to fifteen components per source parent group (placed-art root layer1, children through15), rather than capping a whole car. Separate parent groups can still share the car's true XYZ: this is an unresolved anchor tie, not justification for allowing a car-wide layer counter to cross other objects. No component is silently truncated. General vehicle/peep emitters must obey the same range/cap contract.

Ordinary metal/wood support parents are constrained below the **admitted rail envelope of their own authored track element**. Their original scalar is retained when already lower; otherwise the support gets one integer step below that owned envelope. Other elements' rails never participate, so a high track's column can still occlude an unrelated lower crossing track. Raster offsets and art are unchanged. This removes the prior Looping-flat image-ID-specific centre-anchor exception. Wooden transition `AsOrphan` pieces are different: original `WoodenSupports.cpp` assigns them to the exact rail parent's `Children`, so those sprites share that parent's scalar and retain a bounded overlay layer, rather than being forced behind it.

Added CPU contracts cover every admitted scalar's separation from its adjacent anchor at five world interval bases, all local layers at exponent/range boundaries, foreground/background UI separation, scalar/layer overflow, two valid fifteen-component selected groups versus one invalid sixteen-component group, actual metal support recipes, and a high support crossing an unrelated low rail. No test, shader build or GPU capture has been executed by this review agent. Compilation, focused GPU/UI/filter checks, selected-car checks and all-rotation Everything Park support/track closeups remain pending; the new owner-envelope constraint is not yet visually qualified.
