...
## Superseded depth model: owner correction

The owner's close-up of deployed135 demonstrates the structural error in this checkpoint's plane model: one authored sprite is being depth-tested at different values across its pixels. The new contract is one constant depth per authored sprite component; objects represented by several sprites can assign separate component depths. Terrain planes, billboard gradients, finite surface meshes and pixel-dependent depth are superseded approaches, not pending fixes to pursue.

The correction removes those shader paths and their plane-specific tests. A component's scalar comes from its retained placement plus explicit local layers, with the same depth at every vertex and for its palette-filter fragments. Camera panning, clipping and texel position cannot alter that scalar. Depth-anchor selection remains distinct from image positioning; wrong portal or multi-tile anchors must be corrected as constant component metadata. No per-frame ordering comparator is restored. See [constant component contract](vulkan-constant-component-depth.md) for the current plan and fresh validation gates.
