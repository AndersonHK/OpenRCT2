...
namespace OpenRCT2::Drawing
{
    ...
    {
        if (words.size() < kWorldSelectionHeaderWords
            || words.size() > kWorldSelectionHeaderWords + kWorldSelectionMaximumTiles
        ...
        for (size_t i = 11; i < kWorldSelectionHeaderWords; ++i)
            if (words[i] != 0)
                throw std::invalid_argument("Invalid world selection reserved field");
    ...
    inline std::vector<uint32_t> MakeWorldSelectionWords(
        ...
        std::span<const CoordsXY> selected)
    {
