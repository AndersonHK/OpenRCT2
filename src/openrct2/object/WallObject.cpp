...
namespace OpenRCT2
{
    ...
    void WallObject::ReadLegacy(IReadObjectContext* context, IStream* stream)
    {
        ...
        _legacyType.doorSound = doorSound < Audio::kDoorSoundTypeCount ? static_cast<Audio::DoorSoundType>(doorSound)
                                                                       : Audio::DoorSoundType::none;
    ...
    void WallObject::ReadJson(IReadObjectContext* context, json_t& root)
    {
        ...
        if (!_legacyType.flags.has(WallSceneryFlag::hasPrimaryColour))
        {
            ...
            // Door sound
            _legacyType.doorSound = Audio::DoorSoundType::none;
            auto jDoorSound = properties["doorSound"];
            if (jDoorSound.is_number())
            {
                const auto doorSound = Json::GetNumber<uint32_t>(jDoorSound);
                if (doorSound < Audio::kDoorSoundTypeCount)
