...
namespace OpenRCT2
{
    ...
    money64 RideGetUpkeepCostPerHour(const Ride& ride)
    {
        ...
        return (ride.upkeepCost * GameTime::kTicksPerHour + (GameTime::kGameTicksPerCalendarHalfMonth / 2))
            / GameTime::kGameTicksPerCalendarHalfMonth;
    }
    ...
    static RideRatingAccumulator* RideGetActiveRatingSample(Ride& ride, EntityId sampleEntity, bool create)
    {
        ...
        RideRatingAccumulator* reusable = nullptr;
        ...
        for (size_t i = 0; i < ride.activeRatingSamples.size(); i++)
        {
            ...
            if (create && reusable == nullptr && sample.sampleEntity.IsNull() && !sample.hasSamples())
            {
                reusable = &sample;
                reusableIndex = i;
            }
        ...
        if (reusable == nullptr)
        {
            reusable = &ride.activeRatingSamples.emplace_back();
            reusableIndex = ride.activeRatingSamples.size() - 1;
        }
        reusable->sampleEntity = sampleEntity;
        RideCacheActiveRatingSampleReference(ride, sampleEntity, reusableIndex);
        return reusable;
    }

    RideRatingAccumulator* RideFindActiveRatingSample(Ride& ride, EntityId sampleEntity)
    {
        return RideGetActiveRatingSample(ride, sampleEntity, false);
    }

    RideRatingAccumulator* RideGetOrCreateActiveRatingSample(Ride& ride, EntityId sampleEntity)
    {
        return RideGetActiveRatingSample(ride, sampleEntity, true);
    }
        ...
        std::span<const RideRatingAccumulator> samples, size_t sampleCount)
    {
        ...
        for (size_t i = 0; i < sampleCount; i++)
        {
            ...
            result.transportShelteredDistance += sample.transportShelteredDistance;
            ...
            result.maxPositiveLongitudinalG = std::max(result.maxPositiveLongitudinalG, sample.maxPositiveLongitudinalG);
            result.maxNegativeLongitudinalG = std::min(result.maxNegativeLongitudinalG, sample.maxNegativeLongitudinalG);
        ...
        result.transportShelteredDistance /= static_cast<int64_t>(validSamples);
    ...
    static auto* RideGetRatingLegImpl(TRide& ride, StationIndex originStation, StationIndex destinationStation)
    {
        ...
        const auto it = std::lower_bound(
            ride.ratingLegs.begin(), ride.ratingLegs.end(), key, [](const RideRatingLeg& leg, const auto& candidate) {
                return std::pair{ leg.originStation.ToUnderlying(), leg.destinationStation.ToUnderlying() } < candidate;
            });
        return it != ride.ratingLegs.end() && it->originStation == originStation && it->destinationStation == destinationStation
            ? &*it
            : nullptr;
    }
    ...
    const RideRatingLeg* RideGetRatingLeg(const Ride& ride, StationIndex originStation, StationIndex destinationStation)
    {
        return RideGetRatingLegImpl(ride, originStation, destinationStation);
    }
    ...
    static RideRatingLegMeasurements RideGetRatingLegMeasurementsFromSample(const RideRatingAccumulator& sample)
    {
        return {
            .distanceMetres = ToHumanReadableRideLength(
                static_cast<int32_t>(std::clamp<int64_t>(sample.sampledDistance, 0, std::numeric_limits<int32_t>::max()))),
    ...
    {
        return RideGetRatingLegMeasurementsFromSample(RideGetRecentRatingAccumulator(leg));
    }
            ...
            && sample.destinationStation.ToUnderlying() < ride.numStations && sample.originStation != sample.destinationStation)
        {
                ...
                || it->destinationStation != sample.destinationStation)
            {
                it = ride.ratingLegs.insert(
                    it,
                    RideRatingLeg{ .originStation = sample.originStation, .destinationStation = sample.destinationStation });
            }
            ...
            it->recentSampleCount = static_cast<uint8_t>(std::min<size_t>(it->recentSampleCount + 1, it->recentSamples.size()));
        }
...
#pragma endregion
    ...
    static int32_t RideGetMaxTrainsPerStation(uint8_t stationTiles, int32_t trainLength, uint8_t carsPerTrain)
    {
        ...
        const int64_t stationLength = (static_cast<int64_t>(stationTiles) * kStationLengthPerTile) - kStationLengthEndAllowance;
        ...
        uint8_t carsPerTrain, int32_t numberOfTrains)
    {
        ...
        for (size_t orderIndex = 0; orderIndex < stationCount; orderIndex++)
        {
            ...
            remainingCapacity[stationIndex] = RideGetMaxTrainsPerStation(stationLengths[orderIndex], trainLength, carsPerTrain);
        }
        ...
        bool allowStationDistribution)
    {
        ...
        if (allowStationDistribution && numberOfTrains > 1 && ride.numStations > 1 && ride.mode == RideMode::continuousCircuit
            && !ride.isBlockSectioned() && !ride.flags.has(RideFlag::cableLiftHillComponentUsed))
        {
            ...
            trackCircuitIteratorBegin(&iterator, { trainsPos.x, trainsPos.y, reinterpret_cast<TileElement*>(trackElement) });
    ...
    void Ride::stopGuestsQueuing()
    {
        for (auto peep : EntityList<Guest>())
        {
            ...
            if (peep->State == PeepState::queuing)
            {
                ...
                !peep->CurrentRideStation.IsNull() && peep->CurrentRideStation.ToUnderlying() < numStations
                && RideGetStationPlatformReservation(*this, peep->CurrentRideStation, peep->id).has_value())
            {
    ...
    void Ride::normaliseMazeCapacityMode()
    {
        ...
        operationOption = static_cast<uint8_t>(getMazeCapacityMode());
    }
    ...
    static TransportRideQuality RideGetTransportQualityFromSample(
        const RideRatingAccumulator& sample, const TransportRideQuality& fallback)
    {
    ...
    {
        return RideGetTransportQualityFromSample(RideGetRecentRatingAccumulator(ride), {});
    }

    TransportRideQuality RideGetTransportLegQuality(const RideRatingLeg& leg, const TransportRideQuality& fallback)
    {
        return RideGetTransportQualityFromSample(RideGetRecentRatingAccumulator(leg), fallback);
    }
    ...
    static TransportRideSegment RideGetTransportSegmentForEdge(
        const Ride& ride, StationIndex boardingStation, const RideRatingLeg* measuredLeg, const TransportRideQuality& quality)
    {
        ...
        std::optional<RideRatingAccumulator> legSample;
        RideRatingLegMeasurements legMeasurements{};
        if (measuredLeg != nullptr)
        {
            // Measurements and quality consume the same rolling-history average.
            legSample.emplace(RideGetRecentRatingAccumulator(*measuredLeg));
            legMeasurements = RideGetRatingLegMeasurementsFromSample(*legSample);
        ...
        if (legSample.has_value() && legSample->transportDistance > 0)
        {
            result.shelteredPermille = static_cast<int32_t>(
                std::clamp<int64_t>((legSample->transportShelteredDistance * 1000) / legSample->transportDistance, 0, 1000));
        }
        ...
        const auto segmentQuality = measuredLeg == nullptr ? quality : RideGetTransportQualityFromSample(*legSample, quality);
        ...
        const Ride& ride, StationIndex boardingStation, const TransportRideQuality& quality)
    {
        ...
        if (measuredLeg == nullptr)
        {
            const auto hasAmbiguousMeasuredDeparture = std::any_of(
                ride.ratingLegs.begin(), ride.ratingLegs.end(),
                [boardingStation](const auto& leg) { return leg.originStation == boardingStation && leg.hasSamples(); });
    ...
    static TransportRideSegmentGraph RideBuildTransportSegmentGraph(const Ride& ride, const TransportRideQuality& quality)
    {
        ...
        const TransportRideSegmentGraph& graph, StationIndex boardingStation, StationIndex destinationStation)
    {
        ...
        for (size_t iteration = 0; iteration < stationCount; iteration++)
        {
            ...
            for (size_t candidate = 0; candidate < stationCount; candidate++)
            {
                if (!visited[candidate] && bestTime[candidate] != kUnreachable
                    && (current == stationCount || bestTime[candidate] < bestTime[current]
                        || (bestTime[candidate] == bestTime[current]
                            && (bestFare[candidate] < bestFare[current]
                                || (bestFare[candidate] == bestFare[current]
                                    && bestShelteredTime[candidate] > bestShelteredTime[current])))))
                {
                    current = candidate;
                }
            }
            ...
            for (const auto& segment : graph[current])
            {
                ...
                const auto segmentShelteredTime = (segment.travelTimeMilliseconds / 1000) * segment.shelteredPermille
                    + ((segment.travelTimeMilliseconds % 1000) * segment.shelteredPermille) / 1000;
                const auto candidateShelteredTime = AddClamp<int64_t>(bestShelteredTime[current], segmentShelteredTime);
                const auto candidateFare = AddClamp<money64>(bestFare[current], segment.fareValue);
                if (candidateTime < bestTime[next]
                    || (candidateTime == bestTime[next]
                        && (candidateFare < bestFare[next]
                            || (candidateFare == bestFare[next] && candidateShelteredTime > bestShelteredTime[next]))))
                {
                    bestTime[next] = candidateTime;
                    bestShelteredTime[next] = candidateShelteredTime;
        ...
        return {
            ...
            .shelteredTravelTimeMilliseconds = bestShelteredTime[destination],
    ...
    namespace
    {
        ...
        {
            std::array<std::vector<TransportRideServiceStationRef>, 2> endpoints;
        };
        ...
        struct TransportRideServiceSpatialIndex
        {
            ...
            std::array<std::vector<TransportRideServiceStationRef>, 2> allEndpoints;
        };
        ...
        {
            Guard::Assert(!station.IsNull() && station.ToUnderlying() < cached.stations.size());
        ...
        void TransportServiceUpdateRef(
            std::vector<TransportRideServiceStationRef>& refs, const TransportRideServiceStationRef& ref, bool insert)
        {
            const auto iterator = std::lower_bound(refs.begin(), refs.end(), ref);
            const bool found = iterator != refs.end() && *iterator == ref;
            if (insert && !found)
            {
        ...
        void RideUpdateTransportServiceSpatialIndex(RideId rideId, const CachedTransportRideService& cached, bool insert)
        {
            ...
            for (size_t stationIndex = 0; stationIndex < cached.stations.size(); stationIndex++)
            {
                ...
                const TransportRideServiceStationRef ref{ .ride = rideId, .station = station };
                     ...
                     { TransportRideServiceEndpoint::boardingEntrance, TransportRideServiceEndpoint::destinationExit })
                {
                    ...
                    if (!TransportServiceTryGetSpatialBucket(location, bucketIndex))
                    {
                        continue;
                    }
        ...
        bool RideTransportServiceEndpointsMatch(const CachedTransportRideService& cached, const Ride& ride)
        {
            if (cached.stations.size() != ride.numStations)
            {
                return false;
            }

            const auto locationsMatch = [](const TileCoordsXYZD& left, const TileCoordsXYZD& right) {
        ...
        void RideBuildTransportService(CachedTransportRideService& cached, const Ride& ride)
        {
            const bool endpointsMatch = cached.available && RideTransportServiceEndpointsMatch(cached, ride);
            if (cached.available && !endpointsMatch)
            {
                RideUpdateTransportServiceSpatialIndex(ride.id, cached, false);
            }

            const auto stationCount = static_cast<size_t>(ride.numStations);
            const auto quality = RideGetTransportQuality(ride);
            const auto segmentGraph = RideBuildTransportSegmentGraph(ride, quality);
            ...
            {
                const auto boardingStation = StationIndex::FromUnderlying(
                    static_cast<StationIndex::UnderlyingType>(boardingIndex));
            ...
            if (!endpointsMatch)
            {
                RideUpdateTransportServiceSpatialIndex(ride.id, cached, true);
            }
        }

        void RideDeactivateTransportService(RideId rideId, CachedTransportRideService& cached)
        {
            if (cached.available)
            {
                RideUpdateTransportServiceSpatialIndex(rideId, cached, false);
            }
            cached = {};
            cached.dirty = false;
        }

        void RideRefreshTransportServiceCache()
        {
            ...
            const auto refreshRange = std::max(rideRange, cache.lastRideRange);
            for (size_t rideIndex = 0; rideIndex < refreshRange; rideIndex++)
            {
                ...
                const auto rideId = RideId::FromUnderlying(static_cast<RideId::UnderlyingType>(rideIndex));
                const Ride* ride = rideIndex < rideRange ? &gameState.rides[rideIndex] : nullptr;
                if (ride == nullptr || ride->id.IsNull() || !RideIsAvailableTransportService(*ride))
                {
                    RideDeactivateTransportService(rideId, cached);
                    continue;
                }
                RideBuildTransportService(cached, *ride);
            }
        ...
        void RideQueryTransportServiceStations(
            TransportRideServiceEndpoint endpoint, int64_t rawMinimumX, int64_t rawMinimumY, int64_t rawMaximumX,
            int64_t rawMaximumY, std::vector<TransportRideServiceStationRef>& reusableBuffer)
        {
            ...
            if (hasMapIntersection)
            {
                ...
                for (auto chunkY = minimumChunkY; chunkY <= maximumChunkY; chunkY++)
                {
                    for (auto chunkX = minimumChunkX; chunkX <= maximumChunkX; chunkX++)
                    {
                        ...
                        const auto& refs = spatialIndex.buckets[bucketIndex].endpoints[endpointIndex];
                        for (const auto& ref : refs)
                        {
                            ...
                            if (location.x >= minimumX && location.y >= minimumY && location.x <= maximumX
                                && location.y <= maximumY)
                            {
    ...
    void RideInvalidateTransportServiceCache(RideId rideId)
    {
        ...
        if (rideId.IsNull())
        {
            return; // Preview rides deliberately have no slot in the live service cache.
        }
        Guard::Assert(rideId.ToUnderlying() < _transportRideServiceCache.rides.size());
        _transportRideServiceCache.rides[rideId.ToUnderlying()].dirty = true;
        _transportRideServiceCache.anyDirty = true;
        ...
        StationIndex boardingStation, StationIndex destinationStation) const
    {
        ...
        const auto result = std::lower_bound(
            journeys.begin(), journeys.end(), key, [](const auto& journey, const auto& candidate) {
                return std::pair{ journey.boardingStation.ToUnderlying(), journey.journey.destinationStation.ToUnderlying() }
                < candidate;
            });
    ...
    {
        RideRefreshTransportServiceCache();
        Guard::Assert(!rideId.IsNull() && rideId.ToUnderlying() < _transportRideServiceCache.rides.size());
    ...
    {
        RideQueryTransportServiceStations(
            endpoint, std::min<int64_t>(minimum.x, maximum.x), std::min<int64_t>(minimum.y, maximum.y),
            std::max<int64_t>(minimum.x, maximum.x), std::max<int64_t>(minimum.y, maximum.y), reusableBuffer);
    }
    ...
    namespace
    {
        ...
        public:
            ...
            bool IsSlotOccupied(uint16_t slotIndex) const
            {
                return std::ranges::any_of(
                    assignments, [slotIndex](const auto& assignment) { return assignment.slotIndex == slotIndex; });
            }
        ...
        bool StationSupportsPlatformPreQueue(const Ride& ride, StationIndex stationIndex)
        {
            ...
            const Ride& ride, StationIndex stationIndex, const Vehicle& car, const CarEntry& carEntry, uint8_t seatIndex)
        {
            ...
            if (!carEntry.guestLoadingPositions.empty())
            {
                const auto index = std::min<size_t>(seatIndex, carEntry.guestLoadingPositions.size() - 1);
                loadingPosition = carEntry.guestLoadingPositions[index];
            }
        ...
        uint16_t FindAvailablePlatformSlot(const StationPlatformState& state)
        {
            const auto slot = std::ranges::find_if(
                state.slots, [&](const auto& candidate) { return !state.IsSlotOccupied(candidate.slotIndex); });
    ...
    bool RideSupportsStationPlatformPreQueue(const Ride& ride)
    {
        ...
        const bool supportedTransport = ride.type == RIDE_TYPE_MINIATURE_RAILWAY || ride.type == RIDE_TYPE_MONORAIL
            || ride.type == RIDE_TYPE_SUSPENDED_MONORAIL || ride.type == RIDE_TYPE_CHAIRLIFT;
        if (!supportedTransport && ride.getRideTypeDescriptor().Category != RideCategory::rollerCoaster)
        {
            return false;
        }
    ...
    {
        if (!RideSupportsStationPlatformPreQueue(ride) || !StationSupportsPlatformPreQueue(ride, stationIndex))
        {
            return false;
        }
        ...
        const Ride& ride, StationIndex stationIndex, EntityId guestId)
    {
        ...
        return assignment == state->assignments.end() ? std::nullopt : std::optional{ state->slots[assignment->slotIndex] };
    }
        ...
        const Ride& ride, StationIndex stationIndex, uint8_t trainIndex, Guest& guest)
    {
        ...
        Guard::Assert(trainIndex < ride.numTrains);
        if (ride.getStation(stationIndex).TrainAtStation != trainIndex)
        {
    ...
    {
        Guard::Assert(!ride.id.IsNull() && ride.id.ToUnderlying() < _stationPlatformStates.size());
        _stationPlatformStates[ride.id.ToUnderlying()] = {};
    }
    ...
    void RideRebuildStationPlatformPreQueues()
    {
        ...
        for (auto* guest : EntityList<Guest>())
        {
            ...
            if (guest->CurrentRideStation.IsNull() || guest->CurrentRideStation.ToUnderlying() >= ride->numStations)
            {
                guest->SetState(PeepState::falling);
                continue;
            }
            if (!RideSupportsStationPlatformPreQueue(*ride)
                || !StationSupportsPlatformPreQueue(*ride, guest->CurrentRideStation) || ride->numTrains == 0)
            {
            ...
            if (guest->CurrentCar >= train.carCount)
            {
                guest->recoverFromStationPlatform(*ride);
                continue;
            }
    ...
    uint16_t RideGetTransportStationPlatformCapacity(const Ride& ride, StationIndex stationIndex)
    {
        ...
        if (RideSupportsStationPlatformPreQueue(ride))
        {
            if (ride.numTrains == 0 || ride.vehicles[0].IsNull())
            {
                return 0;
            }
            const auto* head = getGameState().entities.TryGetEntity<Vehicle>(ride.vehicles[0]);
            if (head == nullptr)
            {
                return 0;
            }
            return static_cast<uint16_t>(RideVehicle::StationDetail::BuildTrainSeatSummary(*head).capacity);
        }
    ...
    static uint16_t RideGetTransportStationPlatformOccupancyWithCapacity(
        const Ride& ride, StationIndex stationIndex, uint16_t capacity)
    {
        if (capacity == 0)
        {
            return 0;
    ...
    uint16_t RideGetTransportStationPlatformOccupancy(const Ride& ride, StationIndex stationIndex)
    {
        ...
        return RideGetTransportStationPlatformOccupancyWithCapacity(ride, stationIndex, capacity);
    }

    bool RideIsTransportStationOvercrowded(const Ride& ride, StationIndex stationIndex)
    {
        if (stationIndex.IsNull() || stationIndex.ToUnderlying() >= ride.numStations
            || !ride.getStation(stationIndex).QueueFull)
        {
            return false;
        }

        const auto capacity = RideGetTransportStationPlatformCapacity(ride, stationIndex);
        return capacity > 0 && RideGetTransportStationPlatformOccupancyWithCapacity(ride, stationIndex, capacity) >= capacity;
    }
