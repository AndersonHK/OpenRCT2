...
namespace OpenRCT2
{
    ...
    static constexpr int64_t kVehicleGForceSpeedCouplingDenominator = static_cast<int64_t>(
                                                                          RideRating::kVehicleRatingBaselineSpeed)
    ...
    static constexpr int32_t kRideRatingContextGenerationNeighbourRadius = (kRideRatingContextMaxRadius
                                                                            + kRideRatingContextGenerationChunkSize - 1)
        / kRideRatingContextGenerationChunkSize;
    static constexpr std::array<int32_t, kRideRatingContextMaxRadius + 1> kRideRatingContextDistanceWeights = { 256, 218, 154,
                                                                                                                90,  46,  28,
                                                                                                                18,  12 };
        ...
        const RideRating::LocalContextScore& contextScore, int32_t coefficient, bool isBoatHire)
    {
        ...
        const auto verticalNauseaRaw = isBoatHire ? (verticalTotal * rawScale) / 3
    ...
    static void RideRatingAccumulateLocalContextElement(
        RideRatingLocalContextRaw& raw, const CoordsXYZ& origin, const TileCoordsXY& originTile,
        const TileCoordsXY& candidateTile, int32_t candidateGroundZ, const TileElement& tileElement,
        const RideRatingLocalContextQuery& query)
    {
        ...
        const auto isOrthogonallyAdjacent = distance == 1
            && (candidateTile.x == originTile.x || candidateTile.y == originTile.y);
        ...
        switch (tileElement.getType())
        {
            ...
            case TileElementType::track:
            {
                ...
                {
                    raw.foreignTrackProximity += (75
                                                  * RideRatingGetLocalContextWeightedValue(
                                                      origin, tileElement, distance, false))
                        / kRideRatingContextWeightScale;
                }
                break;
            }
            ...
            default:
            {
                ...
                {
                    raw.scenery += (decorationScore
                                    * RideRatingGetLocalContextWeightedValue(origin, tileElement, distance, true))
                        / kRideRatingContextWeightScale;
                }
                break;
            }
    ...
    {
        const auto chunkX = std::clamp(originTile.x, 0, kMaximumMapSizeTechnical - 1) / kRideRatingContextGenerationChunkSize;
        const auto chunkY = std::clamp(originTile.y, 0, kMaximumMapSizeTechnical - 1) / kRideRatingContextGenerationChunkSize;
        ...
        const RideRating::VehicleLocalContextCache& runtimeCache, const RideRatingLocalContextKey& key)
    {
        return runtimeCache.valid && runtimeCache.originTile.x == key.x && runtimeCache.originTile.y == key.y
            ...
            && static_cast<uint16_t>(runtimeCache.trackType) == key.trackType
    ...
    static RideRating::VehicleRatingEnvironment RideRatingGetLocalContextEnvironment(
        ...
        RideRating::VehicleLocalContextCache* runtimeCache = nullptr)
    {
    ...
    static bool RideRatingGetFixedRideLocalContextBase(const Ride& ride, RideRatingFixedRideLocalContextBase& base)
    {
        ...
        if (rtd.specialType == RtdSpecialType::maze)
        {
            base.location = ride.getStation().getEntrance().toCoordsXY();
            base.baseZ = ride.getStation().getBaseZ();
            return true;
        }

        const auto& station = ride.getStation(stationIndex);
        base.location = station.getStartXY();
    ...
    static CoordsXYZ RideRatingBuildFixedRideLocalContextOrigin(
        const Ride& ride, const RideRatingFixedRideLocalContextBase& base)
    {
        return {
    ...
    void RideRating::InvalidateLocalContextCacheAround(const CoordsXY& location)
    {
        ...
        for (auto originChunkY = minOriginChunkY; originChunkY <= maxOriginChunkY; originChunkY++)
        {
            ...
            {
                _rideRatingLocalContextOriginGenerations[RideRatingGetLocalContextGenerationIndex(originChunkX, originChunkY)]
                    = generation;
            }
        }
    }
    ...
    RideRating::TickScore RideRating::ScorePositiveVerticalGForTick(int32_t verticalG)
    {
        ...
        return {
            .excitement = RideRatingCurveScore(normalPositiveG, 20.0, 4.25)
                + RideRatingCurveScore(excessivePositiveG, 7.5, 2.75),
            .intensity = RideRatingCurveScore(normalPositiveG, 16.0, 4.75)
                + RideRatingCurveScore(excessivePositiveG, 84.0, 3.15),
    ...
    RideRating::VehicleGForceSpeedContext RideRating::PrepareVehicleGForceSpeedContext(int32_t speed, int32_t coupling)
    {
        ...
        return {
            ...
            .couplingNumerator = std::max<int64_t>(
                0,
                kVehicleGForceSpeedCouplingDenominator + normalisedCoupling * (normalisedSpeed - kVehicleRatingBaselineSpeed)),
        };
    }
    ...
    RideRating::TickScore RideRating::VehicleGForceScoreMemo::Get(int32_t verticalG, int32_t lateralG, int32_t longitudinalG)
    {
        ...
        do
        {
                ...
                || (tileElement->asTrack()->getSequenceIndex() == 0 && trackType == tileElement->asTrack()->getTrackType()))
            {
                if (trackType == TrackElemType::endStation)
                {
                    ...
                    if (ride->getStation(entranceIndex).getEntrance().isNull())
                    {
                        state.StationFlags |= RIDE_RATING_STATION_FLAG_NO_ENTRANCE;
                    }
                }
    ...
    static int32_t ride_ratings_get_scenery_score(const Ride& ride)
    {
        ...
        const auto contextScore = RideRating::GetLocalContextScore(
            RideRatingBuildFixedRideLocalContextOrigin(ride, base), ride.id);
...
#pragma region Ride rating calculation helpers
    ...
    static bool RideRatingsGetCompleteLegCompatibility(const Ride& ride, RideRating::Tuple& result)
    {
        ...
        return hasRatings && std::all_of(hasOutboundLeg.begin(), hasOutboundLeg.begin() + ride.numStations, [](bool covered) {
                   return covered;
               });
    }
