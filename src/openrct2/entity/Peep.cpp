    ...
    PeepActionDescription Peep::getActionDescription() const
    {
        switch (state)
        {
            ...
            case PeepState::enteringRide:
            {
                ...
                if (ride != nullptr && state == PeepState::enteringRide)
                {
                    if (rideSubState == PeepRideSubState::approachPlatformSlot)
                        return { PeepActionDescriptionType::walkingToPlatform, currentRide };
                    if (rideSubState == PeepRideSubState::waitingOnPlatform)
                        return { PeepActionDescriptionType::waitingOnPlatform, currentRide };
                }
