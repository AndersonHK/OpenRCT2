...
namespace OpenRCT2::Scripting
{
    ...
    {
        return MakeWithOpaque(
            ctx, new PathNavigatorData{ position, elementIndex, lastDirection, options, GetTileElementRevision(position) });
    }
    ...
    JSValue ScPathNavigator::getCurrent(JSContext* ctx, JSValue thisVal)
    {
        ...
        if (data == nullptr || findPathElement(data) == nullptr)
            return JS_NULL;
    ...
    JSValue ScPathNavigator::moveTo(JSContext* ctx, JSValue thisVal, int argc, JSValue* argv)
    {
        ...
        do
        {
            ...
            data->tileRevision = GetTileElementRevision(data->position);
