...
#pragma pack(push, 1)
    ...
    struct RectCommand
    {
        ...
        enum : uint32_t
        {
            ...
            FLAG_TTF_TEXT = (1u << 5u),
            // A separate asset mask identifies covered zero-index pixels, not raw bitwise masking.
            FLAG_ZERO_COVERAGE = (1u << 6u),
...
#pragma pack(pop)
    ...
    {
        return maxInvocations >= kLightFxComputeLocalSizeX * kLightFxComputeLocalSizeY && maxSizeX >= kLightFxComputeLocalSizeX
            && maxSizeY >= kLightFxComputeLocalSizeY && maxGroupCountZ >= kMaximumLightFxCommandCount;
    }
    ...
    {
        return bounds.x <= bounds.z && bounds.y <= bounds.w && bounds.z >= clip.x && bounds.w >= clip.y && bounds.x < clip.z
            && bounds.y < clip.w;
    }
    ...
    constexpr size_t kWorldSurfaceMaximumRecordCount = static_cast<size_t>(kMaximumMapSizeTechnical) * kMaximumMapSizeTechnical;
    constexpr size_t kWorldSurfaceMaximumChunkCount = (kWorldSurfaceMaximumRecordCount + kWorldSurfaceChunkWidth - 1)
        / kWorldSurfaceChunkWidth;
    ...
    // Qualification scaffold, not a scalable dense-family sorting algorithm.
    constexpr uint32_t kBalloonFixtureCount = 24;
    constexpr uint32_t kBalloonRecordCapacity = 64;
    constexpr uint32_t kBalloonSpriteCount = 13;
    constexpr uint32_t kBalloonMaximumColumns = 129;
    constexpr uint32_t kBalloonMaximumExtent = 4096;

    struct BalloonSpriteTable
    {
        uint64_t revision{};
        std::array<WorldSurfaceSpriteVariant, kBalloonSpriteCount> records{};
    };

    struct BalloonSceneCommand
    {
        std::shared_ptr<const Drawing::RetainedBalloonSnapshot> snapshot;
        std::shared_ptr<const BalloonSpriteTable> sprites;
        Int4 clip{};
        Int2 view{};
        // One viewport category interval; every entity order within it is computed on the GPU.
        uint32_t depthBase{};
        bool sealed{};
    };

    [[nodiscard]] inline uint32_t GetBalloonColumnCount(const BalloonSceneCommand& scene) noexcept
    {
        const auto left = scene.view.x & ~31;
        return static_cast<uint32_t>((scene.view.x + scene.clip.z - scene.clip.x - left + 31) / 32);
    }

    struct TerrainUploadStats
    {
        uint64_t tileBytes{}, materialBytes{}, spriteBytes{}, statusReadbackBytes{};
        uint32_t viewportSubmissions{};
    };

    struct BalloonUploadStats
    {
        uint64_t sourceBytes{}, spriteBytes{};
        uint64_t epoch{}, sequence{}, gpuRevision{};
        uint32_t viewportSubmissions{};
    };
    ...
    struct FrameCommandStream
    {
        ...
        std::vector<BalloonSceneCommand> balloons;
