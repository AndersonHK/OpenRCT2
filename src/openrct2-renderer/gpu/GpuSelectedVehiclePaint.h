...
namespace OpenRCT2::Ui::Gpu
{
    ...
    constexpr uint32_t kSelectedVehiclePaintVersion = 2;
    ...
    struct SelectedVehicleCarRecord
    {
        ...
        uint32_t tileIndex{};
        int32_t x{}, y{}, z{}; // Actual car anchor; component.world preserves only raster projection.
        Int4 coarseCull{};
    };
    ...
    inline void ValidateSelectedVehiclePaintPacket(const SelectedVehiclePaintPacket& packet)
    {
        ...
        if (!packet.source || w.size() < 16 || w[0] != kSelectedVehiclePaintMagic || w[1] != kSelectedVehiclePaintVersion
            || w[2] > kSelectedVehicleMaximumCars || w[3] > kSelectedVehicleMaximumComponents || w[4] != 16
            || w[5] != 16 + w[2] * 12 || w[6] != w[5] + w[3] * 12 || w[7] != w[6] + w[3] * 16 || w[7] != w.size()
            || w[8] != packet.source->sourceTick || (uint64_t(w[9]) | (uint64_t(w[10]) << 32)) != packet.source->worldEpoch
            || (uint64_t(w[11]) | (uint64_t(w[12]) << 32)) != packet.source->entityEpoch || w[13] != 0 || w[14] != 0
            || w[15] != 0)
        ...
        for (uint32_t car = 0; car < w[2]; ++car)
        {
            ...
            if (w[c + 1] == 0 || first != next || count > w[3] - next || (car != 0 && w[c] <= w[c - 12])
                || static_cast<int32_t>(w[c + 5]) < 0 || w[c + 5] > 32767 || static_cast<int32_t>(w[c + 6]) < 0
                || w[c + 6] > 32767 || static_cast<int32_t>(w[c + 7]) < -32768 || static_cast<int32_t>(w[c + 7]) > 32767)
