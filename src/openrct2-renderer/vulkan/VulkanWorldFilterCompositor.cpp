...
namespace OpenRCT2::Ui::Vulkan
{
    ...
    {
        void Barrier(
            VkCommandBuffer cmd, VkPipelineStageFlags src, VkPipelineStageFlags dst, VkAccessFlags reads, VkAccessFlags writes)
        {
        ...
        void ImageBarrier(
            VkCommandBuffer cmd, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout, VkPipelineStageFlags src,
            VkPipelineStageFlags dst, VkAccessFlags reads, VkAccessFlags writes)
        {
    ...
    void WorldFilterCompositor::Initialise(
        const DeviceContext& device, const IndexedResources& resources, const Buffer& status,
        const std::filesystem::path& shaders)
    {
        ...
        if (!features.fragmentStoresAndAtomics || pixels == 0 || pixels * 4 > properties.limits.maxStorageBufferRange
            || groups > properties.limits.maxComputeWorkGroupCount[0] || properties.limits.maxDescriptorSetStorageBuffers < 25
        ...
        _pixels = static_cast<uint32_t>(pixels);
        _groups = static_cast<uint32_t>(groups);
        _capacity = static_cast<uint32_t>(
            std::min({ pixels * 2, uint64_t(0x00fffffe), uint64_t(properties.limits.maxStorageBufferRange / 8) }));
        if (_capacity == 0)
            throw std::runtime_error("World filter compositor has no node capacity");
        ...
        for (uint32_t i = 0; i < 8; i++)
            bindings[i] = { i,
                            i == 4 || i == 5 || i == 7 ? VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER
        ...
        const std::array poolSizes{ VkDescriptorPoolSize{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, frames * 5 },
                                    VkDescriptorPoolSize{ VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, frames * 3 } };
        const VkDescriptorPoolCreateInfo pool{
            VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO, nullptr, 0, frames, uint32_t(poolSizes.size()), poolSizes.data()
        };
        CheckVk(vkCreateDescriptorPool(_device, &pool, nullptr, &_pool), "world filter descriptor pool");
        for (uint32_t frame = 0; frame < frames; frame++)
        {
            const VkDescriptorSetAllocateInfo allocation{ VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, nullptr, _pool, 1,
                                                          &_layout };
            ...
            const std::array buffers{ VkDescriptorBufferInfo{ _heads.GetBuffer(), 0, _heads.GetSize() },
                                      VkDescriptorBufferInfo{ _nodes.GetBuffer(), 0, _nodes.GetSize() },
                                      VkDescriptorBufferInfo{ _control.GetBuffer(), 0, 20 },
                                      VkDescriptorBufferInfo{ status.GetBuffer(), 0, status.GetSize() },
                                      VkDescriptorBufferInfo{ _output.GetBuffer(), 0, _output.GetSize() } };
            const std::array images{
                VkDescriptorImageInfo{ resources.GetNearestSampler(), resources.GetIndexedCanvas(frame).GetView(),
                                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
                VkDescriptorImageInfo{ resources.GetNearestSampler(), resources.GetRemapPalette().GetView(),
                                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
                VkDescriptorImageInfo{ resources.GetNearestSampler(), resources.GetBlendPalette().GetView(),
                                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL }
            };
            ...
            {
                writes[i] = {
                    VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, _sets[frame], i, 0, 1, bindings[i].descriptorType
                };
                if (i == 4 || i == 5 || i == 7)
                    writes[i].pImageInfo = &images[i == 7 ? 2 : i - 4];
                else
                    writes[i].pBufferInfo = &buffers[i == 6 ? 4 : i];
            }
        ...
        const VkPipelineLayoutCreateInfo pipelineLayout{
            VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO, nullptr, 0, 1, &_layout, 1, &push
        };
        ...
        CheckVk(result, "world filter resolve pipeline");
    }
    ...
    void WorldFilterCompositor::Begin(VkCommandBuffer cmd) const
    {
        ...
        Barrier(
            cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    {
        Barrier(
            cmd, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
        ...
        for (uint32_t phase = 0; phase < 2; phase++)
        {
            ...
            Barrier(
                ...
                VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_TRANSFER_BIT, VK_ACCESS_SHADER_WRITE_BIT,
                VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_TRANSFER_READ_BIT);
        }
        ImageBarrier(
            cmd, _images.at(frame), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
            VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
            VK_PIPELINE_STAGE_TRANSFER_BIT, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
            VK_ACCESS_TRANSFER_WRITE_BIT);
        const VkBufferImageCopy copy{ 0,           0,
                                      0,           { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 },
                                      { 0, 0, 0 }, { _extent.width, _extent.height, 1 } };
        vkCmdCopyBufferToImage(cmd, _output.GetBuffer(), _images.at(frame), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy);
        ImageBarrier(
            cmd, _images.at(frame), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
            VK_PIPELINE_STAGE_TRANSFER_BIT,
