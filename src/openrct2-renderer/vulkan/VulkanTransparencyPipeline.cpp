...
namespace OpenRCT2::Ui::Vulkan
{
    ...
    bool TransparencyPipeline::Record(
        const SubmissionToken& frame, const Gpu::CommandBatch<Gpu::RectCommand>& commands, uint32_t layerCount) const
    {
        if (commands.empty())
        {
            return false;
        }
        if (frame.frameIndex >= _frameCount || layerCount == 0)
        {
    ...
    void TransparencyPipeline::CreateDescriptors(const IndexedResources& resources)
    {
        ...
        for (uint32_t frame = 0; frame < _frameCount; frame++)
        {
