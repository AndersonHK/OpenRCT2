...
namespace OpenRCT2::Ui::Vulkan
{
        ...
        const Gpu::CommandBatch<Gpu::SpriteCommand>& sprites) const
    {
        ...
        const auto stage = [&](const auto& batch, const char* error) {
            ...
            if (!batch.empty())
            {
                ...
                allocation.RecordHostWrite(Drawing::UploadMetric::directVertex);
            }
            return allocation;
        };
    ...
    void RectPipeline::RecordSpritesIndirect(const SubmissionToken& frame, const Buffer& sprites,
        const Buffer& indirect, uint32_t drawCount, VkDeviceSize spriteStride, VkDeviceSize indirectOffset, uint32_t indirectStride) const
    {
        ...
        if (_spritePipeline == VK_NULL_HANDLE || frame.commandBuffer == VK_NULL_HANDLE || frame.frameIndex >= _frameCount
            || sprites.GetBuffer() == VK_NULL_HANDLE || sprites.GetSize() < sizeof(Gpu::SpriteCommand)
            || spriteStride < sizeof(Gpu::SpriteCommand) || spriteStride % sizeof(Gpu::SpriteCommand) != 0
            || static_cast<VkDeviceSize>(drawCount) > sprites.GetSize() / spriteStride
            || indirect.GetBuffer() == VK_NULL_HANDLE || indirectStride < sizeof(VkDrawIndirectCommand)
            || indirectStride % 4 != 0 || indirectOffset % 4 != 0
            || indirectOffset > indirect.GetSize()
            || sizeof(VkDrawIndirectCommand) > indirect.GetSize() - indirectOffset
            || static_cast<VkDeviceSize>(drawCount - 1) * indirectStride
                > indirect.GetSize() - indirectOffset - sizeof(VkDrawIndirectCommand))
            throw std::invalid_argument("Invalid device-generated sprite draw buffers");
        const VkRenderPassBeginInfo begin{
            .sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
            .renderPass = _renderPass, .framebuffer = _framebuffers[frame.frameIndex],
            .renderArea = { .offset = { 0, 0 }, .extent = _extent },
        };
        vkCmdBeginRenderPass(frame.commandBuffer, &begin, VK_SUBPASS_CONTENTS_INLINE);
        const VkViewport viewport{ 0.0f, 0.0f, static_cast<float>(_extent.width), static_cast<float>(_extent.height), 0.0f, 1.0f };
        const VkRect2D scissor{ .offset = { 0, 0 }, .extent = _extent };
        const ScreenConstants screen{ static_cast<int32_t>(_extent.width), static_cast<int32_t>(_extent.height) };
        vkCmdSetViewport(frame.commandBuffer, 0, 1, &viewport);
        vkCmdSetScissor(frame.commandBuffer, 0, 1, &scissor);
        vkCmdBindDescriptorSets(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, _pipelineLayout, 0, 1, &_descriptorSet, 0, nullptr);
        vkCmdPushConstants(frame.commandBuffer, _pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(screen), &screen);
        vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, _spritePipeline);
        const auto buffer = sprites.GetBuffer();
        for (uint32_t i = 0; i < drawCount; i++)
        {
            const VkDeviceSize offset = static_cast<VkDeviceSize>(i) * spriteStride;
            vkCmdBindVertexBuffers(frame.commandBuffer, 0, 1, &buffer, &offset);
            vkCmdDrawIndirect(frame.commandBuffer, indirect.GetBuffer(), indirectOffset + static_cast<VkDeviceSize>(i) * indirectStride,
                1, sizeof(VkDrawIndirectCommand));
        }
        vkCmdEndRenderPass(frame.commandBuffer);
    }

    void RectPipeline::RecordDamageClear(const SubmissionToken& frame, std::span<const Gpu::Int4> rectangles) const
    {
        if (rectangles.empty())
            return;
    ...
    void RectPipeline::CreatePipeline()
    {
        ...
        _spritePipeline = create(_shaderDirectory / "indexed_sprite.vert.spv", kSpriteCommandBinding, kSpriteCommandAttributes);
    }
