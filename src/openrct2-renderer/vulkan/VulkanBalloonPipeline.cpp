...
namespace OpenRCT2::Ui::Vulkan
{
    namespace
    {
        ...
        static_assert(sizeof(BalloonConstants) == 36);
        static_assert(offsetof(BalloonConstants, depthBase) == 32);
    ...
    BalloonPipeline::~BalloonPipeline()
    {
        Dispose();
    }

    void BalloonPipeline::Initialise(
        const DeviceContext& device, const IndexedResources& resources, std::filesystem::path shaderDirectory)
    {
        ...
        constexpr VkDeviceSize sourceBytes = Gpu::kBalloonRecordCapacity * sizeof(Drawing::RetainedBalloonRecord);
        constexpr VkDeviceSize spriteBytes = Gpu::kBalloonSpriteCount * sizeof(Gpu::WorldSurfaceSpriteVariant);
        constexpr VkDeviceSize visibleBytes = Gpu::kBalloonMaximumColumns * Gpu::kBalloonRecordCapacity
            * sizeof(Gpu::WorldSurfaceRecord);
        constexpr VkDeviceSize indirectBytes = Gpu::kBalloonMaximumColumns * sizeof(VkDrawIndirectCommand);
    ...
    void BalloonPipeline::Record(const SubmissionToken& frame, const Gpu::BalloonSceneCommand& scene)
    {
        if (frame.frameIndex >= _frameCount)
            throw std::out_of_range("Vulkan balloon frame index is out of range");
        if (!scene.sealed || scene.snapshot == nullptr || scene.sprites == nullptr || scene.snapshot->epoch == 0
            || scene.snapshot->sequence == 0 || scene.snapshot->count != Gpu::kBalloonFixtureCount
            || scene.snapshot->chunks[0] == nullptr || scene.snapshot->chunks[0]->gpuRevision == 0
            || scene.sprites->revision == 0 || scene.clip.z <= scene.clip.x || scene.clip.w <= scene.clip.y
            || GetBalloonColumnCount(scene) > Gpu::kBalloonMaximumColumns)
            throw std::invalid_argument("Vulkan balloon scene is outside diagnostic qualification bounds");
        if (_uploadedEpoch == scene.snapshot->epoch && scene.snapshot->sequence < _lastSequence)
            throw std::invalid_argument("Vulkan balloon scene sequence moved backwards");
        if (_uploadedEpoch != scene.snapshot->epoch)
        {
            _uploadedEpoch = scene.snapshot->epoch;
            _uploadedRevisions.assign(1, 0);
            _uploadedSpriteRevision = 0;
        }
        _lastSequence = scene.snapshot->sequence;
        const auto& chunk = scene.snapshot->chunks[0];
        constexpr VkDeviceSize chunkBytes = Gpu::kBalloonRecordCapacity * sizeof(Drawing::RetainedBalloonRecord);
        const bool hasChangedChunks = _uploadedRevisions[0] != chunk->gpuRevision;
        _stats.epoch = scene.snapshot->epoch;
        _stats.sequence = scene.snapshot->sequence;
        _stats.gpuRevision = chunk->gpuRevision;
        ++_stats.viewportSubmissions;
        ...
        if (hasChangedChunks)
        {
            ...
            if (!allocation)
                throw std::runtime_error("Vulkan upload ring has no room for balloon state deltas");
            std::memcpy(allocation.data, chunk->records.data(), static_cast<size_t>(chunkBytes));
            allocation.RecordHostWrite();
            const VkBufferCopy copy{ allocation.offset, 0, chunkBytes };
            ...
            _stats.sourceBytes += chunkBytes;
            _uploadedRevisions[0] = chunk->gpuRevision;
        }
        if (spritesChanged)
        {
            const VkDeviceSize spriteBytes = scene.sprites->records.size() * sizeof(Gpu::WorldSurfaceSpriteVariant);
            if (spriteBytes != 0)
            {
                auto allocation = frame.upload->Allocate(spriteBytes, alignof(uint32_t), Drawing::UploadCategory::world);
                if (!allocation)
                    throw std::runtime_error("Vulkan upload ring has no room for balloon sprite sets");
            ...
            _stats.spriteBytes += spriteBytes;
        }
        if (hasChangedChunks || spritesChanged)
        {
            std::array<VkBufferMemoryBarrier, 2> barriers{};
        ...
        // Viewport columns are fixed geometry. Counts/order stay GPU-generated; no additional optional device feature.
        for (uint32_t column = 0; column < drawCount; ++column)
        {
            const VkDeviceSize offset = static_cast<VkDeviceSize>(column) * Gpu::kBalloonRecordCapacity
                * sizeof(Gpu::WorldSurfaceRecord);
            vkCmdBindVertexBuffers(frame.commandBuffer, 0, 1, &buffer, &offset);
            vkCmdDrawIndirect(
                frame.commandBuffer, _indirectCommands.GetBuffer(), column * sizeof(VkDrawIndirectCommand), 1,
                sizeof(VkDrawIndirectCommand));
        }
        vkCmdEndRenderPass(frame.commandBuffer);
    }

    void BalloonPipeline::DiscardPendingUploads() noexcept
    {
    ...
    void BalloonPipeline::CreateDescriptors(const IndexedResources& resources)
    {
        ...
        CheckVk(vkCreateDescriptorSetLayout(_device, &layoutInfo, nullptr, &_descriptorSetLayout), "balloons layout");
        ...
        CheckVk(vkCreateDescriptorPool(_device, &poolInfo, nullptr, &_descriptorPool), "balloons pool");
        ...
        CheckVk(vkAllocateDescriptorSets(_device, &allocation, &_descriptorSet), "balloons descriptors");
    ...
    void BalloonPipeline::CreateRenderPass()
    {
        ...
        CheckVk(vkCreateRenderPass(_device, &info, nullptr, &_renderPass), "balloons render pass");
    }

    void BalloonPipeline::CreatePipeline()
    {
        const VkPushConstantRange push{ VK_SHADER_STAGE_COMPUTE_BIT | VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(BalloonConstants) };
        ...
        CheckVk(vkCreatePipelineLayout(_device, &layout, nullptr, &_pipelineLayout), "balloons pipeline layout");
        const auto computeShader = LoadShaderModule(_device, _shaderDirectory / "balloon_order.comp.spv");
        ...
        CheckVk(computeResult, "balloons compute pipeline");
        ...
        _pipeline = CreateGraphicsPipeline(
            ...
            {
                .vertexShader = _shaderDirectory / "balloon.vert.spv",
            ...
            "balloons graphics pipeline");
    }

    void BalloonPipeline::CreateFramebuffers(const IndexedResources& resources)
    {
        for (uint32_t i = 0; i < _frameCount; i++)
        {
            ...
            CheckVk(vkCreateFramebuffer(_device, &info, nullptr, &_framebuffers[i]), "balloons framebuffer");
        }
    }
} // namespace OpenRCT2::Ui::Vulkan
