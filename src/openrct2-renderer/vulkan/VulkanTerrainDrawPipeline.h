...
namespace OpenRCT2::Ui::Vulkan
{
    ...
    class TerrainDrawPipeline final
    {
        ...
        PeepFieldPipeline _peeps;
        RectPipeline _rect;
        std::array<Buffer, 4> _buffers;
        VkExtent3D _extent{};
        uint32_t _frameCount{}, _assetCapacity{}, _columnCapacity{};
        std::array<VkDeviceSize, 4> _bufferSizes{};
    ...
    public:
        ...
        void Record(
            const SubmissionToken& frame, const Gpu::Terrain::RetainedTerrainSnapshot& snapshot,
        ...
        uint64_t GetLastPeepUploadBytes() const noexcept
        {
            return _peeps.GetLastUploadBytes();
        }
        void DiscardPendingUploads() noexcept; // Mandatory for abandoned recordings.
        uint64_t GetLastSpriteUploadBytes() const noexcept
        {
            return _lastSpriteUploadBytes;
        }
        const Buffer& GetParentBuffer() const noexcept
        {
            return _buffers[1];
        }
        const Buffer& GetCommandBuffer() const noexcept
        {
            return _buffers[2];
        }
        const Buffer& GetColumnBuffer() const noexcept
        {
            return _buffers[3];
        }
        const TerrainEmissionPipeline& GetEmission() const noexcept
        {
            return _emission;
        }
    };
