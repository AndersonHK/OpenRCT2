...
namespace OpenRCT2::Ui::Vulkan
{
    ...
    namespace
    {
        struct Parameters
        {
            ...
            int32_t depthBase;
            uint32_t spriteCount, columnCount, pass, assetCount, limits;
        };
        static_assert(sizeof(Parameters) == 64 && offsetof(Parameters, columnCount) == 48);
        ...
        constexpr std::array<VkDeviceSize, 4> kBufferSizes = {
            Terrain::kDrawSpriteCapacity * sizeof(Terrain::DrawSpriteMetadata),
            VkDeviceSize{ Terrain::kDrawMaximumColumns } * Terrain::kDrawColumnCapacity * sizeof(Terrain::DrawParent),
            Terrain::kDrawMaximumColumns * kSpritesPerColumn,
            Terrain::kDrawMaximumColumns * sizeof(Terrain::DrawColumnStatus),
        };
        ...
        void Barrier(VkCommandBuffer command, VkPipelineStageFlags fromStage, VkAccessFlags fromAccess,
            VkPipelineStageFlags toStage, VkAccessFlags toAccess)
        {
            const VkMemoryBarrier barrier{ .sType = VK_STRUCTURE_TYPE_MEMORY_BARRIER,
                .srcAccessMask = fromAccess, .dstAccessMask = toAccess };
        ...
        void ValidateTable(const Terrain::DrawSpriteTable& table, uint32_t assetCount)
        {
            ...
            {
                if ((!first && sprite.imageIndex <= previous) || sprite.width < 1 || sprite.width > 2048
                    || sprite.height < 1 || sprite.height > 2048 || sprite.xOffset < -32768 || sprite.xOffset > 32767
                    || sprite.yOffset < -32768 || sprite.yOffset > 32767 || sprite.flags != 0
                    || sprite.reserved0 != 0 || sprite.reserved1 != 0)
                    throw std::invalid_argument("Unsupported terrain sprite metadata");
                first = false; previous = sprite.imageIndex;
                for (int32_t zoom = 0; zoom < 2; zoom++)
                {
                    ...
                    if (variant.width < 1 || variant.width > 2048 || variant.height < 1 || variant.height > 2048
                        || variant.xOffset < -32768 || variant.xOffset > 32767
                        || variant.yOffset < -32768 || variant.yOffset > 32767
                        || variant.flags > 1 || variant.asset >= assetCount
                        || variant.effectiveZoom < 0 || variant.effectiveZoom > 1
                        || variant.coordinateShift < 0 || variant.coordinateShift > 1
                        || variant.effectiveZoom + variant.coordinateShift != zoom)
    ...
    TerrainDrawPipeline::~TerrainDrawPipeline() { Dispose(); }

    void TerrainDrawPipeline::Initialise(const DeviceContext& device, const IndexedResources& resources,
        ...
        const std::filesystem::path& columnShader)
    {
        ...
        try
        {
            ...
            if (limits.maxPerStageDescriptorStorageBuffers < 8 || limits.maxDescriptorSetStorageBuffers < 8
                || limits.maxComputeWorkGroupCount[0] < Terrain::kDrawMaximumColumns
                || limits.maxComputeSharedMemorySize < 2002 * sizeof(int32_t)
                || limits.maxStorageBufferRange < *std::max_element(kBufferSizes.begin(), kBufferSizes.end())
            ...
            for (size_t i = 0; i < _buffers.size(); i++)
            {
                ...
                if (i == 2) usage |= VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
                if (i == 3) usage |= VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT;
                _buffers[i].Initialise(device.GetPhysicalDevice(), _device, kBufferSizes[i], usage);
            }
            ...
            CheckVk(vkCreateDescriptorSetLayout(_device, &descriptors, nullptr, &_descriptorLayout), "terrain draw descriptors");
            ...
            const std::array<const Buffer*, 8> buffers = { &_emission.GetTileBuffer(), &_emission.GetTileCountBuffer(),
                &_emission.GetPrimitiveBuffer(), &_emission.GetStatusBuffer(), &_buffers[0], &_buffers[1], &_buffers[2], &_buffers[3] };
            ...
            const VkComputePipelineCreateInfo pipeline{ .sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO,
                .stage = { .sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
                    .stage = VK_SHADER_STAGE_COMPUTE_BIT, .module = module, .pName = "main" }, .layout = _layout };
    ...
    void TerrainDrawPipeline::Dispose()
    {
        ...
        _extent = {}; _frameCount = 0; _assetCapacity = 0; _epoch = 0;
    ...
    void TerrainDrawPipeline::DiscardPendingUploads() noexcept
    {
        ...
        _spriteRevision = 0; _assetCount = 0; _spriteCount = 0;
    }

    void TerrainDrawPipeline::Record(const SubmissionToken& frame, const Terrain::RetainedTerrainSnapshot& snapshot,
        const Terrain::DrawSpriteTable& sprites, const Terrain::DrawCamera& camera, uint32_t assetCount)
    {
        if (_pipeline == VK_NULL_HANDLE || frame.commandBuffer == VK_NULL_HANDLE || frame.upload == nullptr
            || frame.frameIndex >= _frameCount || camera.width < 1 || camera.width > 1024
            || camera.height < 1 || camera.height > 1024 || camera.x < -1048576 || camera.x > 1048576
            || camera.y < -1048576 || camera.y > 1048576 || camera.rotation >= 4 || camera.zoom < 0 || camera.zoom > 1
            || camera.transparent > 1 || camera.stableSort > 1 || camera.clipX < 0 || camera.clipY < 0
            ...
            || static_cast<uint64_t>(camera.clipY) + camera.height > _extent.height
            || camera.depthBase < 1 || static_cast<uint32_t>(camera.depthBase) >= (1u << 22) - Terrain::kDrawColumnCapacity
            || sprites.revision == 0 || sprites.records.empty() || sprites.records.size() > Terrain::kDrawSpriteCapacity
            || assetCount == 0 || assetCount > _assetCapacity)
            throw std::invalid_argument("Unsupported retained terrain drawing parameters");
        const int32_t columnWidth = 32 >> camera.zoom;
        const auto columnCount = static_cast<uint32_t>((camera.x + camera.width - 1 - FloorTo(camera.x, columnWidth)) / columnWidth + 1);
        if (columnCount > Terrain::kDrawMaximumColumns)
            throw std::invalid_argument("Terrain draw column capacity exceeded");
        if (_epoch != snapshot.worldEpoch) { _epoch = snapshot.worldEpoch; DiscardPendingUploads(); }
        _lastSpriteUploadBytes = 0;
        const bool uploadSprites = _spriteRevision != sprites.revision;
        if (uploadSprites) ValidateTable(sprites, assetCount);
        ...
        try
        {
            ...
            if (uploadSprites)
            {
                ...
                if (!allocation) throw std::runtime_error("Upload ring cannot hold terrain sprite metadata generation");
                ...
                _spriteRevision = sprites.revision; _assetCount = assetCount;
                _spriteCount = static_cast<uint32_t>(sprites.records.size());
            }
            Barrier(frame.commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT | VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_SHADER_WRITE_BIT,
                VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT);
            Parameters parameters{ camera.x, camera.y, camera.width, camera.height, camera.clipX, camera.clipY,
                camera.rotation, camera.zoom, camera.transparent, camera.stableSort, camera.depthBase,
                _spriteCount, columnCount, 0, assetCount, camera.parentCapacity | (camera.commandCapacity << 16) };
            ...
            _rect.RecordSpritesIndirect(frame, _buffers[2], _buffers[3], columnCount, kSpritesPerColumn,
