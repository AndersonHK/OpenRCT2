...
namespace OpenRCT2::Ui::Vulkan
{
    ...
    bool LightFxPipeline::IsSupported(const DeviceContext& device, Gpu::Extent logicalExtent) noexcept
    {
        ...
        const SubmissionToken& frame, const Gpu::LightFxFrameSnapshot& snapshot, IndexedResources& resources) const
    {
        ...
        if (!snapshot.lights.empty())
        {
            ...
            allocation = frame.upload->Allocate(byteSize, alignof(uint32_t), Drawing::UploadCategory::lightFx);
            if (!allocation)
                return false;
            std::memcpy(allocation.data, snapshot.lights.data(), static_cast<size_t>(byteSize));
            allocation.RecordHostWrite(Drawing::UploadMetric::directStorage);
        }
