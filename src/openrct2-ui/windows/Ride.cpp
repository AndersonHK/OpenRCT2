...
#pragma region Measurements
        ...
        void MeasurementsOnPrepareDraw()
        {
            ...
            const bool hasRatingLeg = !gTrackDesignSaveMode && ride->numStations > 1 && GetSelectedRatingLeg(*ride) != nullptr;
            widgets[WIDX_RATING_LEG].setVisible(hasRatingLeg);
            widgets[WIDX_RATING_LEG_DROPDOWN].setVisible(hasRatingLeg);

            widgets[WIDX_SAVE_TRACK_DESIGN].tooltip = STR_SAVE_TRACK_DESIGN_NOT_POSSIBLE;
            const bool savingDesign = gTrackDesignSaveMode && gTrackDesignSaveRideIndex == rideId;
            widgets[WIDX_SAVE_TRACK_DESIGN].setVisible(!savingDesign);
            widgets[WIDX_SELECT_NEARBY_SCENERY].setVisible(savingDesign);
            widgets[WIDX_RESET_SELECTION].setVisible(savingDesign);
            widgets[WIDX_SAVE_DESIGN].setVisible(savingDesign);
            widgets[WIDX_CANCEL_DESIGN].setVisible(savingDesign);
            if (!savingDesign)
            {
        ...
        void MeasurementsOnDraw(RenderTarget& rt)
        {
            ...
            if (widgets[WIDX_RATING_LEG].isVisible())
            {
                const auto* ride = GetRide(rideId);
...
#pragma region Graphs
        ...
        void GraphsOnPrepareDraw()
        {
            ...
            widgets[WIDX_GRAPH_LONGITUDINAL].setVisible(hasGForces);
...
#pragma region Income
        ...
        void IncomeOnPrepareDraw()
        {
            ...
            widgets[WIDX_PRIMARY_PRICE_INCREASE].setVisible();
            widgets[WIDX_PRIMARY_PRICE_DECREASE].setVisible(!primaryUsesTargetPricing);
