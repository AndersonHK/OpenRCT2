...
#pragma region Widgets
    ...
    static_assert(kRideConstructionWidgets.size() <= 64, "Construction widget state uses 64-bit masks");
...
#pragma endregion
    ...
    public:
        ...
        void updateWidgets()
        {
            ...
            // Restore the base layout before applying this track's seat-rotation layout.
            widgets[WIDX_BANKING_GROUPBOX].text = STR_RIDE_CONSTRUCTION_ROLL_BANKING;
            widgets[WIDX_BANKING_GROUPBOX].right = kGroupWidth + 2;
            widgets[WIDX_SPEED_GROUPBOX].right = kGroupWidth + 2;
            for (int32_t i = WIDX_BANK_LEFT; i <= WIDX_BANK_RIGHT; i++)
                widgets[i].moveToX(kRideConstructionWidgets[i].left);
            resizeSpinner(WIDX_SPEED_SETTING_SPINNER, { 12, 123 + widgets[WIDX_TITLE].bottom }, { 85, kSpinnerHeight });
