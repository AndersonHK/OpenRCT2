...
namespace OpenRCT2::Ui::Windows
{
    ...
    public:
        StepDirection direction = StepDirection::previous;
        ...
        void onPrepareDraw() override
        {
            ...
            bool stepVisible = gLegacyScene != LegacyScene::trackDesignsManager
                ...
                && EnumValue(gameState.editorStep) < std::size(kNextButtonMouseUp);
            if (stepVisible)
            {
        ...
        {
            if (widgetIndex != WIDX_STEP_BUTTON)
                return;
            onPrepareDraw();
            if (!widgets[WIDX_STEP_BUTTON].isVisible())
                return;
            auto& gameState = getGameState();
            const auto index = EnumValue(gameState.editorStep);
            const auto callback = direction == StepDirection::previous ? kPreviousButtonMouseUp[index]
                                                                       : kNextButtonMouseUp[index];
            if (callback != nullptr)
                callback();
        }
    ...
    WindowBase* editorStepControllerOpen()
    {
        ...
        {
            if (auto* next = windowMgr->FindByNumber(WindowClass::editorStepController, 1); next != nullptr)
                return next;
