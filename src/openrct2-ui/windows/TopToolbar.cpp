...
#pragma endregion
    ...
    public:
        ...
        {
            // Network restrictions must preserve controls already hidden by settings or editor mode.
            switch (Network::GetMode())
            {
                case Network::Mode::none:
                    widgets[WIDX_NETWORK].setHidden();
                    widgets[WIDX_CHAT].setHidden();
                    break;
                case Network::Mode::client:
                    widgets[WIDX_PAUSE].setHidden();
                    [[fallthrough]];
        ...
        {
            if (windowPos.x != 0 || width != ContextGetWidth())
            {
                invalidate();
                windowPos.x = 0;
        ...
        void AlignButtonsCentre()
        {
            ...
            if (windowPos.x != xPos || width != totalWidth)
            {
                invalidate();
                windowPos.x = xPos;
                width = totalWidth;
